...
$(function(){
	...
	$("#prevBtn").click(function(){
		if($("#prevtb_num").val() == -1)
			...
			location.href="/viewMenteeBoard.do?tb_num="+$("#prevtb_num").val()+"&thisPage="+$("#thisPage").val();
	});
	/**
	 * 다음 게시글 버튼을 눌렀을 때,
	...
	$("#nextBtn").click(function(){
		if($("#nexttb_num").val() == -1)
			...
			location.href="/viewMenteeBoard.do?tb_num="+$("#nexttb_num").val()+"&thisPage="+$("#thisPage").val();
	});
	/**
	 * 목록 버튼을 눌렀을 때, 게시판 목록을 보여준다.
	 * 해당 게시판으로 들어오기 전에, 게시판 페이징 번호를 thisPage로 가져왔다가
	 * 그 값을 통해 다시 들어오기 전의 상황과 똑같은 페이지를 보게끔 한다.
	 */
	$("#listBtn").click(function(){
		location.href="/menteeBoard.do?page_num="+$("#thisPage").val();
	});
	/**
	...
	$("#modifyBtn").click(function(){
		location.href="/modifyMenteeBoard.do?tb_num="+$("#tb_num").val()+"&thisPage="+$("#thisPage").val();
	});
	/**
	...
	$("#deleteBtn").click(function(){
		...
		if(execute)
			location.href="/deleteMenteeBoard.do?tb_num="+$("#tb_num").val()+"&thisPage="+$("#thisPage").val();
	});
})