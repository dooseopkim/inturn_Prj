...
(function() {
	...
	nhn.husky.HuskyCore = jindo.$Class({
		...
		setDebugger : function(e) {
			this.htOptions.oDebugger = e;e.setApp(this)
		},
		exec : function(g, f, e) {
			...
			this.exec = this._exec;this.exec(g, f, e)
		},
		...
		_execWithDebugger : function(h, g, f) {
			...
			this.htOptions.oDebugger.log_MessageEnd(h, g);return e
		},
		...
		_doExec : function(m, j, h) {
			...
			if (this.oLazyMessage[m]) {
				...
				this._loadLazyFiles(m, j, h, l.aFilenames, 0);return false
			}
		...
		registerPlugin : function(f) {
			...
			this.exec("MSG_PLUGIN_REGISTERED", [ f ]);return f.nIdx
		},
		...
		run : function(h) {
			this.htRunOptions = h || {};this._changeAppStatus(nhn.husky.APP_STATUS.WAITING_FOR_PLUGINS_READY);
		...
		_execMsgStepWithDebugger : function(e, g, h) {
			...
			this.htOptions.oDebugger.log_MessageStepEnd(e, g, h);return f
		},
		...
		_execHandlerWithDebugger : function(k, h, g) {
			...
			try {
				...
				f = k[h].apply(k, g);this.aCallerStack.pop()
			} catch (j) {
				this.htOptions.oDebugger.handleException(j);
				f = false
			} this.htOptions.oDebugger.log_CallHandlerEnd(k, h, g);return f
		},
		_execHandlerWithoutDebugger : function(h, g, f) {
			this.aCallerStack.push(h);
			var e = h[g].apply(h, f);
			this.aCallerStack.pop();return e
		},
		...
		_loadLazyFiles : function(f, l, g, k, j) {
			var h = k.length;
			if (h <= j) {
				this.oLazyMessage[f] = null;this.oApp.exec(f, l, g);return
			}
			var e = k[j];
			if (d[e]) {
				...
				jindo.LazyLoading.load(nhn.husky.SE2M_Configuration.LazyLoad.sJsBaseURI + "/" + e, jindo.$Fn(function(n, r, o, q, p) {
					var m = q[p];
					d[m] = 1;this._loadLazyFiles(n, r, o, q, p + 1)
...
nhn.CurrentSelection_IE = function() {
	...
	this.isCollapsed = function() {
		this._oSelection = this._document.selection;return this._oSelection.type == "None"
	}
};
...
nhn.W3CDOMRange = jindo.$Class({
	...
	cloneContents : function() {
		...
		if (d) {
			...
			var g;
			while (c) {
				g = c.nextSibling;e.appendChild(c);
				c = g
			}
		}
	...
	_constructClonedTree : function(b, d) {
		...
		var g = function(l, k, j) {
			...
			j.insertBefore(h, j.firstChild);return m
		};
		this._recurConstructClonedTree = g;
		b[b.length] = nhn.DOMFix.parentNode(b[b.length - 1]);this._recurConstructClonedTree(b, b.length - 1, d);return {
			oStartContainer : a,
			oEndContainer : c
		}
	},
	...
	_copyRange : function(a) {
		...
		a._document = this._document;return a
	},
	collapse : function(a) {
		if (a) {
	...
	compareBoundaryPoints : function(a, b) {
		...
		case nhn.W3CDOMRange.START_TO_START:
			return this._compareEndPoint(this.startContainer, this.startOffset, b.startContainer, b.startOffset);case nhn.W3CDOMRange.START_TO_END:
			return this._compareEndPoint(this.endContainer, this.endOffset, b.startContainer, b.startOffset);case nhn.W3CDOMRange.END_TO_END:
			return this._compareEndPoint(this.endContainer, this.endOffset, b.endContainer, b.endOffset);case nhn.W3CDOMRange.END_TO_START:
	...
	insertNode : function(b) {
		...
		a.elParent.insertBefore(b, a.elBefore);this.setStartBefore(b)
	},
	selectNode : function(a) {
		this.reset(this._window);this.setStartBefore(a);this.setEndAfter(a)
	},
	selectNodeContents : function(a) {
		this.reset(this._window);this.setStart(a, 0, true);this.setEnd(a, nhn.DOMFix.childNodes(a).length)
	},
	...
	setEndAfter : function(a, b) {
		...
		if (a.tagName == "BODY") {
			this.setEnd(a, nhn.DOMFix.childNodes(a).length, true, b);return
		}
	...
	setStartAfter : function(a, b) {
		...
		if (a.tagName == "BODY") {
			this.setStart(a, nhn.DOMFix.childNodes(a).length, true, b);return
		}
	...
	setStartBefore : function(a, b) {
		...
		if (a.tagName == "BODY") {
			this.setStart(a, 0, true, b);return
		}
		this.setStart(nhn.DOMFix.parentNode(a), this._getPosIdx(a), true, b)
	},
	surroundContents : function(a) {
		a.appendChild(this.extractContents());this.insertNode(a);this.selectNode(a)
	},
	toString : function() {
		var a = this._document.createElement("DIV");
		a.appendChild(this.cloneContents());return a.textContent || a.innerText || ""
	},
	...
	_updateRangeInfo : function() {
		if (!this.startContainer) {
			this.reset(this._window);return
		}
	...
	_splitTextEndNodes : function(a) {
		...
		a = this._splitEndTextNode(a);return a
	},
...
nhn.HuskyRange = jindo.$Class({
	...
	$init : function(a) {
		...
		this.rxHasBlock = new RegExp("(?:<(?:" + this.sBlockElement + "|" + this.sBlockContainer + ").*?>|style=[\"']?[^>]*?(?:displays?:s?block)[^>]*?[\"']?)", "gi");this.setWindow(a)
	},
	select : function() {
		try {
	...
	setRange : function(a, b) {
		this.reset(this._window);this.setStart(a.startContainer, a.startOffset, b, true);this.setEnd(a.endContainer, a.endOffset, b)
	},
	setEndNodes : function(b, a) {
		this.reset(this._window);this.setEndAfter(a, true);this.setStartBefore(b)
	},
	...
	moveToXPathBookmark : function(b) {
		...
		this.endOffset = a.nOffset;return true
	},
	...
	placeStringBookmark_NonWebkit : function() {
		...
		f.id = this.HUSKY_BOOMARK_END_ID_PREFIX + b;c.insertNode(f);
		var c = this.cloneRange();
		c.collapseToStart();
		var a = this._document.createElement("SPAN");
		a.id = this.HUSKY_BOOMARK_START_ID_PREFIX + b;c.insertNode(a);
		if (jindo.$Agent().navigator().ie) {
			try {
		...
		this.moveToBookmark(b);return b
	},
	placeStringBookmark_Webkit : function() {
		...
		var c = this.cloneRange();
		c.collapseToEnd();
		f = this._document.createTextNode("");c.insertNode(f);
		d = f.parentNode;
		...
		e.id = this.HUSKY_BOOMARK_END_ID_PREFIX + b;d.insertBefore(e, f);
		var c = this.cloneRange();
		c.collapseToStart();
		f = this._document.createTextNode("");c.insertNode(f);
		d = f.parentNode;
		...
		var a = this._document.createElement("SPAN");
		a.id = this.HUSKY_BOOMARK_START_ID_PREFIX + b;d.insertBefore(a, f);this.moveToBookmark(b);return b
	},
	cloneRange : function() {
	...
	moveToStringBookmark : function(c, b) {
		...
		if (b) {
			this.setEndAfter(d);this.setStartBefore(a)
		} else {
			this.setEndBefore(d);this.setStartAfter(a)
		}
		return true
	},
	removeStringBookmark : function(a) {
		this._removeAll(this.HUSKY_BOOMARK_START_ID_PREFIX + a);this._removeAll(this.HUSKY_BOOMARK_END_ID_PREFIX + a)
	},
	...
	surroundContentsWithNewNode : function(a) {
		var b = this._document.createElement(a);
		this.surroundContents(b);return b
	},
	...
	isNodeInRange : function(d, c, b) {
		...
		if (b && d.firstChild) {
			a.setStartBefore(d.firstChild);a.setEndAfter(d.lastChild)
		} else {
			a.selectNode(d)
		}
	...
	pasteHTML : function(e) {
		...
		if (o.oLineBreaker && o.oLineBreaker.nodeName === "P" && l.rxHasBlock.test(e)) {
			...
			l.removeStringBookmark(m);this._removeEmptyP(this._getPrevElement(g));
			...
			if (a) {
				...
				a.innerHTML = "\uFEFF";c.appendChild(a)
			}
			this.selectNodeContents(a);this.collapseToStart()
		} else {
			var h = g.firstChild;
			var j = g.lastChild;
			...
			this.setEndNodes(h, j);l.moveToBookmark(m);l.deleteContents();l.removeStringBookmark(m)
		}
	},
	...
	toString : function() {
		this.toString = nhn.W3CDOMRange.prototype.toString;return this.toString()
	},
	toHTMLString : function() {
		var a = this._document.createElement("DIV");
		a.appendChild(this.cloneContents());return a.innerHTML
	},
	...
	_checkTextDecoration : function(e) {
		...
		while (b && b.tagName !== "BODY") {
			if (b.nodeType !== 1) {
				b = nhn.DOMFix.parentNode(b);continue
			}
			if (!c && this._hasTextDecoration(b, "underline")) {
				c = true;this._setTextDecoration(e, "underline")
			}
			if (!a && this._hasTextDecoration(b, "line-through")) {
				a = true;this._setTextDecoration(e, "line-through")
			}
			if (c && a) {
				return
			}
	...
	styleRange : function(k, f, b, a, e) {
		...
		this.reset(this._window);this.setStartBefore(j[0]);this.setEndAfter(j[j.length - 1])
	},
	...
	_getStyleParentNodes : function(c, o) {
		...
		for (var p = 0; p < f; p++) {
			...
			j = this._document.createElement("SPAN");a.insertBefore(j, h);j.appendChild(h);
			d[q++] = j;
			if (c) {
				j.setAttribute(c, "true")
			}
		}
		this.setStartBefore(n);this.setEndAfter(b);return d
	},
...
nhn.BrowserSelection = function(a) {
	...
	this.selectRange = function(b) {
		this.selectNone();this.addRange(b)
	};
...
nhn.BrowserSelectionImpl_FF = function() {
	...
	this.compareEndPoints = function(g, d, c, a) {
		...
		f.setStart(g, d);b.setStart(c, a);f.collapse(true);b.collapse(true);try {
	...
	this._FFRange2W3CRange = function(b) {
		...
		a.setStart(b.startContainer, b.startOffset, true);a.setEnd(b.endContainer, b.endOffset, true);return a
	};
...
nhn.BrowserSelectionImpl_IE = function() {
	...
	this.getRangeAt = function(b) {
		...
		if (this._oSelection.type == "Control") {
			...
			a.selectNode(d);return a
		} else {
			c = this._oSelection.createRange();
			var d = c.parentElement();
			if (!d || d.ownerDocument != this._document) {
				a = new nhn.W3CDOMRange(this._window);return a
			}
			a = this._IERange2W3CRange(c);return a
		}
	};
	this.addRange = function(a) {
	...
	this._W3CRange2IERange = function(a) {
		...
		b.setEndPoint("EndToEnd", c);this._updateLastRange(b, a);return b
	};
	this._getIERangeAt = function(f, d) {
		...
		c.moveToElementText(e);c.collapse(b.bCollapseToStart);c.moveStart("character", a);return c
	};
	...
	this._IERange2W3CRange = function(d) {
		...
		var c = null;
		var b = null;
		c = d.duplicate();c.collapse(true);
		b = this._getW3CContainerAndOffset(c, true);a.setStart(b.oContainer, b.iOffset, true, true);
		var e = d.duplicate();
		e.collapse(true);
		if (e.isEqual(d)) {
			a.collapse(true)
		} else {
			c = d.duplicate();c.collapse(false);
			b = this._getW3CContainerAndOffset(c);a.setEnd(b.oContainer, b.iOffset, true)
		}
		this._updateLastRange(d, a);return a
	};
	this._getW3CContainerAndOffset = function(c, f) {
		...
		if (n !== 0 && k[n - 1].nodeType == 3) {
			...
			var d = null;
			if (g) {
				a.moveToElementText(g);a.collapse(false);
				d = g.nextSibling
			} else {
				a.moveToElementText(o);a.collapse(true);
				d = o.firstChild
			}
			var m = p.duplicate();
...
nhn.husky.CorePlugin = jindo.$Class({
	...
	_loadFullPlugin : function(f, a, e, c, d, b) {
		jindo.LazyLoading.load(nhn.husky.SE2M_Configuration.LazyLoad.sJsBaseURI + "/" + f[b], jindo.$Fn(function(o, h, n, l, m, k) {
			...
			var g = o[0];
			if (k == o.length - 1) {
				this.htLazyLoadRequest_plugins[g].nStatus = 2;this.oApp.exec("MSG_FULL_PLUGIN_LOADED", [ o, h, n, l, m ]);return
			}
...
nhn.husky.HuskyRangeManager = jindo.$Class({
	...
	$BEFORE_MSG_APP_READY : function() {
		if (this.oWindow && this.oWindow.tagName == "IFRAME") {
			this.oWindow = this.oWindow.contentWindow;nhn.CurrentSelection.setWindow(this.oWindow)
		}
		this.oApp.exec("ADD_APP_PROPERTY", [ "getSelection", jindo.$Fn(this.getSelection, this).bind() ]);this.oApp.exec("ADD_APP_PROPERTY", [ "getEmptySelection", jindo.$Fn(this.getEmptySelection, this).bind() ])
	},
...
nhn.husky.SE2M_Toolbar = jindo.$Class({
	...
	$LOCAL_BEFORE_FIRST : function(a) {
		...
		this.elLastToolbarItem = c[b - 1];this.oApp.registerBrowserEvent(this.toolbarArea, "keydown", "NAVIGATE_TOOLBAR", [])
	},
	$init : function(b, a) {
		this._htOptions = a || {};
		this.htUIList = {};
		this.htWrappedUIList = {};
		this.aUICmdMap = {};this._assignHTMLElements(b)
	},
	$ON_MSG_APP_READY : function() {
		if (this.oApp.bMobile) {
			this.oApp.registerBrowserEvent(this.toolbarArea, "touchstart", "EVENT_TOOLBAR_TOUCHSTART")
		} else {
			this.oApp.registerBrowserEvent(this.toolbarArea, "mouseover", "EVENT_TOOLBAR_MOUSEOVER");this.oApp.registerBrowserEvent(this.toolbarArea, "mouseout", "EVENT_TOOLBAR_MOUSEOUT")
		}
		this.oApp.registerBrowserEvent(this.toolbarArea, "mousedown", "EVENT_TOOLBAR_MOUSEDOWN");this.oApp.exec("ADD_APP_PROPERTY", [ "getToolbarButtonByUIName", jindo.$Fn(this.getToolbarButtonByUIName, this).bind() ]);
		var a = jindo.$$.getSingle(".se2_tool");
		this.oApp.exec("REGISTER_HOTKEY", [ "esc", "FOCUS_EDITING_AREA", [], a ]);
		if (this._htOptions.aDisabled) {
			this._htOptions._sDisabled = "," + this._htOptions.aDisabled.toString() + ",";this.oApp.exec("DISABLE_UI", [ this._htOptions.aDisabled ])
		}
	},
	$ON_NAVIGATE_TOOLBAR : function(b) {
		...
		if ((b.element == this.elLastToolbarItem) && (b.key().keyCode == a)) {
			if (b.key().shift) {
				...
				this.elFirstToolbarItem.focus();b.stopDefault()
			}
		}
		if (b.element == this.elFirstToolbarItem && (b.key().keyCode == a)) {
			if (b.key().shift) {
				b.stopDefault();this.elLastToolbarItem.focus()
			}
		}
	},
	...
	$ON_SHOW_TOOLBAR_ACTIVE_LAYER : function(a, d, c, b) {
		this.oApp.exec("SHOW_ACTIVE_LAYER", [ a, d, c ]);this.oApp.exec("POSITION_TOOLBAR_LAYER", [ a, b ])
	},
	...
	$ON_SELECT_UI : function(a) {
		...
		b.removeClass("hover");b.addClass("active")
	},
	...
	_enableUI : function(d) {
		...
		if (this.aUICmdMap[d]) {
			for (b = 0; b < this.aUICmdMap[d].length; b++) {
				f = this.aUICmdMap[d][b];this.oApp.exec("ENABLE_MESSAGE", [ f ])
			}
		}
	},
	_disableUI : function(d) {
		...
		e.addClass("off");e.removeClass("hover");
		var a = g.getElementsByTagName("BUTTON");
		for (b = 0, c = a.length; b < c; b++) {
			a[b].disabled = true
		}
		...
		if (this.aUICmdMap[d]) {
			for (b = 0; b < this.aUICmdMap[d].length; b++) {
				f = this.aUICmdMap[d][b];this.oApp.exec("DISABLE_MESSAGE", [ f ])
			}
		}
	},
...
nhn.husky.SE_EditingAreaManager = jindo.$Class({
	...
	$init : function(g, f, a, e, d) {
		...
		this.elContentsField = jindo.$(f);this._assignHTMLElements(d);
	...
	$ON_MSG_APP_READY : function() {
		...
		this.iframeWindow = this.oApp.getWYSIWYGWindow();this.oApp.exec("REGISTER_CONVERTERS", []);this.oApp.exec("CHANGE_EDITING_MODE", [ this.sDefaultEditingMode, true ]);this.oApp.exec("LOAD_CONTENTS_FIELD", [ false ]);this.oApp.exec("REGISTER_HOTKEY", [ "esc", "CLOSE_LAYER_POPUP", [], document ]);
	...
	$ON_LOAD_CONTENTS_FIELD : function(b) {
		...
		a = a.replace(/^\s+/, "");this.oApp.exec("SET_CONTENTS", [ a, b ])
	},
	...
	$AFTER_CHANGE_EDITING_MODE : function(a, b) {
		if (this._oPrevActivePlugin) {
			...
			this.oApp.exec("SET_IR", [ c ]);this._setEditingAreaDimension()
		}
		this.startAutoResize();
		if (!b) {
	...
	$ON_MSG_EDITING_AREA_RESIZE_STARTED : function() {
		...
		this._fitElementInEditingArea(this.elEditingAreaContainer);this.oApp.exec("STOP_AUTORESIZE_EDITING_AREA");this.oApp.exec("SHOW_EDITING_AREA_COVER");
	...
	$ON_RESIZE_EDITING_AREA : function(b, a) {
		...
		this._fitElementInEditingArea(this.elResizingBoard);this._setEditingAreaDimension()
	},
	...
	$ON_MSG_EDITING_AREA_RESIZE_ENDED : function(a, b, c) {
		...
		this.elEditingAreaContainer.style.overflow = "";this._setEditingAreaDimension()
	},
	...
	_setEditingAreaDimension : function(a) {
		a = a || this.oActivePlugin;this._fitElementInEditingArea(a.elEditingArea)
	},
	...
	attachDocumentEvents : function(a) {
		this.oApp.registerBrowserEvent(a, "click", "EVENT_EDITING_AREA_CLICK");this.oApp.registerBrowserEvent(a, "dblclick", "EVENT_EDITING_AREA_DBLCLICK");this.oApp.registerBrowserEvent(a, "mousedown", "EVENT_EDITING_AREA_MOUSEDOWN");this.oApp.registerBrowserEvent(a, "mousemove", "EVENT_EDITING_AREA_MOUSEMOVE");this.oApp.registerBrowserEvent(a, "mouseup", "EVENT_EDITING_AREA_MOUSEUP");this.oApp.registerBrowserEvent(a, "mouseout", "EVENT_EDITING_AREA_MOUSEOUT");this.oApp.registerBrowserEvent(a, "mousewheel", "EVENT_EDITING_AREA_MOUSEWHEEL");this.oApp.registerBrowserEvent(a, "keydown", "EVENT_EDITING_AREA_KEYDOWN");this.oApp.registerBrowserEvent(a, "keypress", "EVENT_EDITING_AREA_KEYPRESS");this.oApp.registerBrowserEvent(a, "keyup", "EVENT_EDITING_AREA_KEYUP");this.oApp.registerBrowserEvent(a, "scroll", "EVENT_EDITING_AREA_SCROLL")
	},
	createCoverDiv : function() {
		this.elResizingBoard = document.createElement("DIV");this.elEditingAreaContainer.insertBefore(this.elResizingBoard, this.elEditingAreaContainer.firstChild);
		...
		this.elResizingBoard.style["-khtml-opacity"] = 0;this._fitElementInEditingArea(this.elResizingBoard);
	...
	getContents : function() {
		...
		a = this._cleanContents(a);return a
	},
...
nhn.husky.SE_EditingArea_WYSIWYG = jindo.$Class({
		...
		if (this.oApp.oNavigator.ie) {
			jindo.$Fn(function(b) {
				...
				if (a && a.type.toLowerCase() === "control" && b.key().keyCode === 8) {
					this.oApp.exec("EXECCOMMAND", [ "delete", false, false ]);b.stop()
				}
				this._bIERangeReset = false
			}, this).attach(this.iframe.contentWindow.document, "keydown");jindo.$Fn(function(a) {
				this._oIERange = null;
				this._bIERangeReset = true
	...
	_setScrollbarWidth : function() {
		...
		b.style.top = "-9999px";a.body.appendChild(b);
		this.nScrollbarWidth = b.offsetWidth - b.clientWidth;a.body.removeChild(b)
	},
	...
	startAutoResize : function() {
		...
		this._setAutoResize();
		this.nCheckBodyInterval = setInterval(this.fnCheckBodyChange, 500);this.oApp.exec("START_FLOAT_TOOLBAR")
	},
	stopAutoResize : function() {
		this.bAutoResize = false;clearInterval(this.nCheckBodyInterval);this.oApp.exec("STOP_FLOAT_TOOLBAR");jindo.$Element(this.getDocument().body).css({
			overflow : "visible",
			overflowY : "visible"
	...
	_checkBodyChange : function() {
		...
		if (a !== this.nBodyLength) {
			this.nBodyLength = a;this._setAutoResize()
		}
	},
	_getResizeHeight : function() {
		var d = this.getDocument().body,
			b,
			f,
			a = [ "width", "fontFamily", "fontSize", "fontWeight", "fontStyle", "lineHeight", "letterSpacing", "textTransform", "wordSpacing" ],
			e,
			c;
		...
		f = this.elDummy.scrollHeight;return f
	},
	_setAutoResize : function() {
		...
		this.iframe.style.height = d + "px";this.oApp.welEditingAreaContainer.height(d);
		if (!this.oApp.oNavigator.msafari) {
	...
	_setBodyHeight : function() {
		...
		if (a === 0) {
			l.css("height", d + "px");setTimeout(this.fnSetBodyHeight, 500);return
		}
		...
		this.nBodyHeight_last = j;setTimeout(this.fnSetBodyHeight, 500)
	},
	...
	$ON_START_CHECKING_BODY_HEIGHT : function() {
		if (this.bStopCheckingBodyHeight) {
			this.bStopCheckingBodyHeight = false;this.fnSetBodyHeight()
		}
	},
	...
	$ON_CHANGE_EDITING_MODE : function(a, b) {
		if (a === this.sMode) {
			...
			this.iframe.style.visibility = "hidden";this.oApp.exec("STOP_CHECKING_BODY_HEIGHT")
		}
	},
	$AFTER_CHANGE_EDITING_MODE : function(a, b) {
		this._oIERange = null
	...
	$ON_REFRESH_WYSIWYG : function() {
		...
		this._disableWYSIWYG();this._enableWYSIWYG()
	},
	...
	$ON_IE_HIDE_CURSOR : function() {
		...
		if (a && a.createRange) {
			try {
				...
				a = this.oApp.getSelection();a.select();a.oBrowserSelection.selectNone()
			}
		} else {
	...
	_backspace : function(b) {
		...
		if (!!c && c.nodeType === 1 && c.tagName === "TABLE") {
			jindo.$Element(c).leave();b.stop(jindo.$Event.CANCEL_ALL)
		}
	},
	...
	$ON_PASTE_HTML : function(b, g, e) {
		...
		if (!j.ie) {
			...
			this.oApp.getWYSIWYGDocument().body.innerHTML = this.oApp.getWYSIWYGDocument().body.innerHTML;c.moveToBookmark(f);c.collapseToEnd();c.select();c.removeStringBookmark(f);
			c = this.oApp.getSelection();
			if (!!g) {
				g.setRange(c)
			}
		} else {
			c.collapseToEnd();c.select();
			this._oIERange = null;
			this._bIERangeReset = false
		}
		if (b.indexOf("<img") > -1) {
	...
	$ON_FOCUS_N_CURSOR : function(d, a) {
		...
		if (!c.collapsed) {
			...
			if (d) {
				this.oApp.exec("FOCUS");
				b = this.getDocument().body;c.selectNode(b);c.collapseToEnd();c.select();this._scrollIntoView(b)
			} else {
				this.oApp.exec("FOCUS")
			}
		}
	...
	_getElementVerticalPosition : function(b) {
		...
		d.nTop = a;
		d.nBottom = a + jindo.$Element(b).height();return d
	},
	...
	_scrollIntoView : function(c) {
		...
		if ((a = d.nBottom - b.nBottom) > 0) {
			this.getWindow().scrollTo(0, b.nTop + a);return
		}
	...
	$AFTER_MSG_EDITING_AREA_RESIZE_ENDED : function(a, b, d) {
		...
		if (!jindo.$Agent().navigator().ie) {
			...
			c.moveToBookmark(this.sBM);c.select();c.removeStringBookmark(this.sBM)
		}
	},
	...
	_pageUp : function(c) {
		...
		this.oApp.getWYSIWYGWindow().scrollTo(0, a);c.stop()
	},
...
nhn.husky.SE_EditingArea_HTMLSrc = jindo.$Class({
	...
	startAutoResize : function() {
		...
		this.bAutoResize = true;
		this.AutoResizer = new nhn.husky.AutoResizer(this.elEditingArea, a);this.AutoResizer.bind()
	},
	stopAutoResize : function() {
...
TextRange.prototype.getSelection = function() {
	...
	if (isNaN(this._o.selectionStart)) {
		...
		var c = null;
		c = this._oDoc.selection.createRange().duplicate();a.moveToElementText(d);c.collapse(true);a.setEndPoint("EndToEnd", c);
		b[0] = a.text.length;
		c = this._oDoc.selection.createRange().duplicate();a.moveToElementText(d);c.collapse(false);a.setEndPoint("EndToEnd", c);
		b[1] = a.text.length;d.blur()
	} else {
...
TextRange.prototype.setSelection = function(d, a) {
	...
	if (c.setSelectionRange) {
		...
		if (c.createTextRange) {
			...
			b.collapse(true);b.moveStart("character", d);b.moveEnd("character", a - d);b.select();c.blur()
		}
	}
};
...
nhn.husky.SE_EditingArea_TEXT = jindo.$Class({
	...
	_addLineBreaker : function(b) {
		...
		for (var a = 0; a < e; a++) {
			...
			if (d !== null && d !== "") {
				c.append("<P>");c.append(f[a]);c.append("</P>")
			} else {
				if (!jindo.$Agent().navigator().ie) {
					c.append("<P><BR></P>")
				} else {
...
nhn.husky.SE_EditingAreaVerticalResizer = jindo.$Class({
	...
	$init : function(b, a) {
		this.htConversionMode = a;this._assignHTMLElements(b)
	},
	$BEFORE_MSG_APP_READY : function() {
		...
		if (this.oApp.bMobile) {
			...
			if (this.isUseVerticalResizer()) {
				...
				if (!!this.welNoticeLayer && !Number(jindo.$Cookie().get(this.sCookieNotice))) {
					this.welNoticeLayer.delegate("click", "BUTTON.bt_clse", jindo.$Fn(this._closeNotice, this).bind());this.welNoticeLayer.show()
				}
				...
				this.$FnMouseOut = jindo.$Fn(this._mouseout, this);this.$FnMouseDown.attach(this.oResizeGrip, "mousedown");this.$FnMouseOver.attach(this.oResizeGrip, "mouseover");this.$FnMouseOut.attach(this.oResizeGrip, "mouseout")
			} else {
	...
	checkResizeGripPosition : function(b) {
		...
		if (b) {
			...
			this.oApp.exec("MSG_EDITING_AREA_RESIZE_STARTED");this.oApp.exec("RESIZE_EDITING_AREA_BY", [ 0, (-1) * c ]);this.oApp.exec("MSG_EDITING_AREA_RESIZE_ENDED")
		}
	...
	_mouseover : function(a) {
		a.stopBubble();this.welConversionMode.addClass("controller_on")
	},
	_mouseout : function(a) {
		a.stopBubble();this.welConversionMode.removeClass("controller_on")
	},
	_mousedown : function(a) {
		this.iStartHeight = a.pos().clientY;
		this.iStartHeightOffset = a.pos().layerY;this.$FnMouseMove.attach(document, "mousemove");this.$FnMouseUp.attach(document, "mouseup");
		this.iStartHeight = a.pos().clientY;this.oApp.exec("HIDE_ACTIVE_LAYER");this.oApp.exec("HIDE_ALL_DIALOG_LAYER");this.oApp.exec("MSG_EDITING_AREA_RESIZE_STARTED", [ this.$FnMouseDown, this.$FnMouseMove, this.$FnMouseUp ])
	},
	...
	_closeNotice : function() {
		this.welNoticeLayer.hide();jindo.$Cookie().set(this.sCookieNotice, 1, 365 * 10)
	}
});
nhn.husky.SE_WYSIWYGEnterKey = jindo.$Class({
	...
	$ON_MSG_APP_READY : function() {
		...
		this.tmpTextNode = this.oSelection._document.createTextNode(unescape("%u00A0"));jindo.$Fn(this._onKeyDown, this).attach(this.oApp.getWYSIWYGDocument(), "keydown")
	},
	...
	_addBlankText : function(c) {
		...
		for (b = 0, e = g.length; b < e; b++) {
			...
			if (!d || (d.nodeType == 3 && nhn.husky.SE2M_Utils.isBlankTextNode(d)) || (d.nodeType == 1 && f.childNodes.length == 1 && (d.id.indexOf(c.HUSKY_BOOMARK_START_ID_PREFIX) > -1 || d.id.indexOf(c.HUSKY_BOOMARK_END_ID_PREFIX) > -1))) {
				a = c._document.createTextNode(unescape("%uFEFF"));f.appendChild(a)
			}
		}
	},
	...
	_wrapBlock : function(l, n) {
		...
		if (!o.bParentBreak || d.rxBlockContainer.test(o.oLineBreaker.tagName)) {
			l.stop();d.deleteContents();
			if (!!o.oNode.parentNode && o.oNode.parentNode.nodeType !== 11) {
				m = this.oApp.getWYSIWYGDocument().createElement(this.sLineBreaker);d.moveToBookmark(k);d.setStartBefore(o.oNode);this._addBlankText(d);d.surroundContents(m);d.collapseToEnd();
				f = this.oApp.getWYSIWYGDocument().createElement(this.sLineBreaker);
				var c = this._getBlockEndNode(o.oNode, e.oNode);
				if (c === o.oNode) {
					c = e.oNode
				}
				d.setEndAfter(c);this._addBlankText(d);d.surroundContents(f);d.moveToStringBookmark(k, true);d.collapseToEnd();d.removeStringBookmark(k);d.select();
				...
				d.selectNodeContents(j);d.collapseToStart();d.select();this.oApp.exec("CHECK_STYLE_CHANGE");
				k = d.placeStringBookmark();setTimeout(jindo.$Fn(function(q) {
					var r = d.getStringBookmark(q);
					if (!r) {
						return
					}
					d.moveToStringBookmark(q);d.select();d.removeStringBookmark(q)
				}, this).bind(k), 0);return
			}
		}
		...
		if (this.htBrowser.firefox) {
			if (h && h.nextSibling && h.nextSibling.tagName == "IFRAME") {
				...
				setTimeout(jindo.$Fn(function(q) {
					...
					this._convertHeadSpace(r);d.removeStringBookmark(q)
				}, this).bind(k), 0)
			}
		} else {
			if (this.htBrowser.ie) {
				var g = h.parentNode,
					...
					a = false;
				if (!h || !g) {
					d.removeStringBookmark(k);return
				}
				...
				if (b || a) {
					setTimeout(jindo.$Fn(this._addTextDecorationTag, this).bind(b, a), 0);return
				}
				setTimeout(jindo.$Fn(this._addExtraCursorHolder, this).bind(g), 0)
			} else {
				this._convertHeadSpace(h);d.removeStringBookmark(k)
			}
		}
	},
	_addExtraCursorHolder : function(h) {
		...
		f.selectNodeContents(b);f.collapseToStart();f.select()
	},
	_addSpace : function(b) {
		...
		if (!d) {
			a = this.tmpTextNode.cloneNode();b.appendChild(a)
		}
		return b
	},
	_addTextDecorationTag : function(e, a) {
		...
		while (d) {
			if (d.nodeType === 3) {
				d = nhn.DOMFix.parentNode(d);break
			}
		...
		if (e) {
			b = c._document.createElement("U");d.appendChild(b);
			d = b
		}
		if (a) {
			b = c._document.createElement("STRIKE");d.appendChild(b)
		}
		b.innerHTML = "\u200B";c.selectNodeContents(b);c.collapseToEnd();c.select()
	},
	...
	_insertBR : function(b) {
		...
		e.insertNode(f);e.selectNode(f);e.collapseToEnd();
		if (!this.htBrowser.ie) {
			var c = e.getLineInfo();
			var d = c.oEnd;
			if (d.bParentBreak) {
				...
				if (a === 0) {
					e.pasteHTML("<br type='_moz'/>");e.collapseToEnd()
				}
			}
		}
		e.insertNode(this.oApp.getWYSIWYGDocument().createTextNode(""));e.select()
	}
});
nhn.husky.SE2M_EditingModeChanger = jindo.$Class({
	...
	$ON_MSG_APP_READY : function() {
		if (this.oApp.htOptions.bOnlyTextMode) {
			...
			this.elTEXTButton.style.display = "block";this.oApp.exec("CHANGE_EDITING_MODE", [ "TEXT" ])
		} else {
			this.oApp.registerBrowserEvent(this.elWYSIWYGButton, "click", "EVENT_CHANGE_EDITING_MODE_CLICKED", [ "WYSIWYG" ]);this.oApp.registerBrowserEvent(this.elHTMLSrcButton, "click", "EVENT_CHANGE_EDITING_MODE_CLICKED", [ "HTMLSrc" ]);this.oApp.registerBrowserEvent(this.elTEXTButton, "click", "EVENT_CHANGE_EDITING_MODE_CLICKED", [ "TEXT", false ]);this.showModeChanger();
	...
	$ON_CHANGE_EDITING_MODE : function(a) {
		if (a == "HTMLSrc") {
			this.welWYSIWYGButtonLi.removeClass("active");this.welHTMLSrcButtonLi.addClass("active");this.welTEXTButtonLi.removeClass("active");
			this.elWYSIWYGButton.disabled = false;
			this.elHTMLSrcButton.disabled = true;
			this.elTEXTButton.disabled = false;this.oApp.exec("HIDE_ALL_DIALOG_LAYER");this.oApp.exec("DISABLE_ALL_UI", [ {
				aExceptions : [ "mode_switcher" ]
			} ])
		} else {
			if (a == "TEXT") {
				this.welWYSIWYGButtonLi.removeClass("active");this.welHTMLSrcButtonLi.removeClass("active");this.welTEXTButtonLi.addClass("active");
				this.elWYSIWYGButton.disabled = false;
				this.elHTMLSrcButton.disabled = false;
				this.elTEXTButton.disabled = true;this.oApp.exec("HIDE_ALL_DIALOG_LAYER");this.oApp.exec("DISABLE_ALL_UI", [ {
					aExceptions : [ "mode_switcher" ]
				} ])
			} else {
				this.welWYSIWYGButtonLi.addClass("active");this.welHTMLSrcButtonLi.removeClass("active");this.welTEXTButtonLi.removeClass("active");
				this.elWYSIWYGButton.disabled = true;
				this.elHTMLSrcButton.disabled = false;
				this.elTEXTButton.disabled = false;this.oApp.exec("RESET_STYLE_STATUS");this.oApp.exec("ENABLE_ALL_UI", [])
			}
		}
	}
});
nhn.husky.SE_PasteHandler = jindo.$Class({
	...
	_handlePaste : function(d) {
		...
		if (a) {
			this._preparePaste();setTimeout(jindo.$Fn(function() {
				var m = new RegExp("^[\u200b]");
				if (this.zwspStart) {
				...
				} k.moveToStringBookmark(this._sBM);k.collapseToEnd();k.select();k.removeStringBookmark(this._sBM)
			}, this).bind(), 0)
		}
	},
	...
	_securePasteArea : function() {
		...
		e.innerHTML = "\u200b";c.setStartAfter(e);c.setEndBefore(this.zwspEnd);c.select()
	},
		...
		if (this._sTarget) {
			try {
				if (!!this.elPasteHelper) {
					this._clearPasteHelper();this._showPasteHelper()
				} else {
					this._createPasteHelper()
				}
				this._loadToPasteHelper();this._loadToBody();this._hidePasteHelper()
			} catch (a) {
	...
	_savePastedContents : function() {
		var c = this.oApp.getSelection();
		c.moveToStringBookmark(this._sBM);c.select();
		this.oSelectionClone = c.cloneContents();c.collapseToEnd();c.select();
	...
	_filterPastedContents : function(V, C) {
		...
		this._doFilter = jindo.$Fn(function(ay) {
			...
			z = "";
			r = "";
			l = 0;K;M = "", c = "", x = "", d = "", ac = "", n = "", aj = false, W = false;
			while (ay != "") {
				g = "", s = "";
				if (al.test(ay)) {
					s = ay.match(al)[0];
	...
	_filterTableContents : function(f) {
		...
		function C() {
			...
			if ((c !== 0) && (h !== 0) && (c == h)) {
				m = b.index;d.push(m + 8);j.push(l());k()
			}
			o()
		}
		var l = function() {
		...
		if (this.htBrowser.ie) {
			...
			var q,
				w;
			if ((e > 0) && (e == r)) {
				q = y[0];t.push(n.slice(0, q));
				for (var x = 0, z = y.length; x < z; x++) {
					if ((e > 1) && (x > 0)) {
						q = d[x - 1];
						w = y[x];t.push(n.slice(q, w))
					}
					q = y[x];
					w = d[x];t.push(j[x])
				}
				w = d[r - 1];t.push(n.slice(w, n.length + 1));return t.join("")
			} else {
				return n
			}
		} else {
	...
	_createPasteHelper : function() {
		if (!this.elPasteHelper) {
			...
			this.elPasteHelper.style.left = "9999px";this.elEditingAreaContainer.appendChild(this.elPasteHelper)
		}
	},
	...
	_loadToBody : function() {
		...
		try {
			h.moveToStringBookmark(this._sBM);h.select();var p = h.getNodes();
			...
			o.setStartBefore(this.elPasteHelper.firstChild);o.setEndAfter(this.elPasteHelper.lastChild);o.select();var g = o.getNodes();
			...
			h.moveToStringBookmark(this._sBM);h.select();h.deleteContents();var a = h.getStringBookmark(this._sBM, true);
...
nhn.husky.SE2M_ExecCommand = jindo.$Class({
	...
	$ON_MSG_APP_READY : function() {
		this.oApp.exec("REGISTER_HOTKEY", [ "ctrl+b", "EXECCOMMAND", [ "bold", false, false ] ]);this.oApp.exec("REGISTER_HOTKEY", [ "ctrl+u", "EXECCOMMAND", [ "underline", false, false ] ]);this.oApp.exec("REGISTER_HOTKEY", [ "ctrl+i", "EXECCOMMAND", [ "italic", false, false ] ]);this.oApp.exec("REGISTER_HOTKEY", [ "ctrl+d", "EXECCOMMAND", [ "strikethrough", false, false ] ]);this.oApp.exec("REGISTER_HOTKEY", [ "meta+b", "EXECCOMMAND", [ "bold", false, false ] ]);this.oApp.exec("REGISTER_HOTKEY", [ "meta+u", "EXECCOMMAND", [ "underline", false, false ] ]);this.oApp.exec("REGISTER_HOTKEY", [ "meta+i", "EXECCOMMAND", [ "italic", false, false ] ]);this.oApp.exec("REGISTER_HOTKEY", [ "meta+d", "EXECCOMMAND", [ "strikethrough", false, false ] ]);this.oApp.exec("REGISTER_HOTKEY", [ "tab", "INDENT" ]);this.oApp.exec("REGISTER_HOTKEY", [ "shift+tab", "OUTDENT" ]);this.oApp.exec("REGISTER_UI_EVENT", [ "bold", "click", "EXECCOMMAND", [ "bold", false, false ] ]);this.oApp.exec("REGISTER_UI_EVENT", [ "underline", "click", "EXECCOMMAND", [ "underline", false, false ] ]);this.oApp.exec("REGISTER_UI_EVENT", [ "italic", "click", "EXECCOMMAND", [ "italic", false, false ] ]);this.oApp.exec("REGISTER_UI_EVENT", [ "lineThrough", "click", "EXECCOMMAND", [ "strikethrough", false, false ] ]);this.oApp.exec("REGISTER_UI_EVENT", [ "superscript", "click", "EXECCOMMAND", [ "superscript", false, false ] ]);this.oApp.exec("REGISTER_UI_EVENT", [ "subscript", "click", "EXECCOMMAND", [ "subscript", false, false ] ]);this.oApp.exec("REGISTER_UI_EVENT", [ "justifyleft", "click", "EXECCOMMAND", [ "justifyleft", false, false ] ]);this.oApp.exec("REGISTER_UI_EVENT", [ "justifycenter", "click", "EXECCOMMAND", [ "justifycenter", false, false ] ]);this.oApp.exec("REGISTER_UI_EVENT", [ "justifyright", "click", "EXECCOMMAND", [ "justifyright", false, false ] ]);this.oApp.exec("REGISTER_UI_EVENT", [ "justifyfull", "click", "EXECCOMMAND", [ "justifyfull", false, false ] ]);this.oApp.exec("REGISTER_UI_EVENT", [ "orderedlist", "click", "EXECCOMMAND", [ "insertorderedlist", false, false ] ]);this.oApp.exec("REGISTER_UI_EVENT", [ "unorderedlist", "click", "EXECCOMMAND", [ "insertunorderedlist", false, false ] ]);this.oApp.exec("REGISTER_UI_EVENT", [ "outdent", "click", "EXECCOMMAND", [ "outdent", false, false ] ]);this.oApp.exec("REGISTER_UI_EVENT", [ "indent", "click", "EXECCOMMAND", [ "indent", false, false ] ]);
	...
	$BEFORE_EXECCOMMAND : function(a, j, g, f) {
		...
		if (/^insertorderedlist|insertunorderedlist$/i.test(a)) {
			this._getDocumentBR();this._checkBlockQuoteCondition_IE()
		}
		if (/^justify*/i.test(a)) {
		...
		if (this.oNavigator.ie && this.oApp.getWYSIWYGDocument().selection) {
			if (this.oApp.getWYSIWYGDocument().selection.type === "Control") {
				b = this.oApp.getSelection();b.select()
			}
		}
		if (a == "insertorderedlist" || a == "insertunorderedlist") {
	...
	_insertDummyParagraph_IE : function(a) {
		this._elDummyParagraph = document.createElement("P");a.appendChild(this._elDummyParagraph)
	},
	...
	$ON_EXECCOMMAND : function(c, m, j) {
		...
		var e = this.oApp.getSelection();
		m = (m == "" || m) ? m : false;
		j = (j == "" || j) ? j : false;this.oApp.exec("IS_SELECTED_TD_BLOCK", [ "bIsSelectedTd", b ]);
		f = b.bIsSelectedTd;
		if (f) {
			if (c == "indent") {
			...
			case "indent":
			case "outdent":
				this.oApp.exec("RECORD_UNDO_BEFORE_ACTION", [ c ]);var d = e.placeStringBookmark();
				if (c === "indent") {
				...
				e.moveToStringBookmark(d);e.select();e.removeStringBookmark(d);setTimeout(jindo.$Fn(function(n) {
			...
			case "justifyfull":
				...
				if (e.collapsed && this._rxCmdInline.test(c)) {
					var h = e.placeStringBookmark(),
						...
						a = k.previousSibling;
					if (!a || a.nodeValue !== "\u200B") {
						a = this.oApp.getWYSIWYGDocument().createTextNode("\u200B");e.insertNode(a)
					}
					e.removeStringBookmark(h);e.selectNodeContents(a);e.select();this.oEditingArea.execCommand(c, m, j);e.collapseToEnd();e.select();
	...
	$AFTER_EXECCOMMAND : function(d, a, c, b) {
		...
		if (/^insertorderedlist|insertunorderedlist$/i.test(d)) {
			this._removeDummyParagraph_IE();this._fixCorruptedBlockQuote(d === "insertorderedlist" ? "OL" : "UL");
	...
	_removeSpanAlign : function() {
		...
		for (var c = 0, e = a.length; c < e; c++) {
			...
			if (b.tagName && b.tagName === "SPAN") {
				b.style.textAlign = "";b.removeAttribute("align")
			}
		}
	},
	...
	_fixAlign : function(d) {
		...
		var h = !this.oNavigator.ie ? function() {} : function(j) {
			if (j.tagName && j.tagName === "TABLE") {
				j.removeAttribute("align");return true
			}
			return false
		};
		if (f.collapsed) {
		...
		for (var e = 0, g = a.length; e < g; e++) {
			...
			if (b && b.align !== b.style.textAlign) {
				b.style.textAlign = d;b.setAttribute("align", d)
			}
		}
	},
	...
	_setBlockExecCommand : function(h, d, g) {
		var b,
			a,
			f = {};
		this.oSelection = this.oApp.getSelection();this.oApp.exec("GET_SELECTED_TD_BLOCK", [ "aTdCells", f ]);
		b = f.aTdCells;
		for (var e = 0; e < b.length; e++) {
			this.oSelection.selectNodeContents(b[e]);this.oSelection.select();
			if (this.oNavigator.firefox) {
	...
	_indentMargin : function(c) {
		...
		if (c.tagName === "LI") {
			...
			if (c.nextSibling && c.nextSibling.tagName && c.nextSibling.tagName === c.parentNode.tagName) {
				c.nextSibling.insertBefore(c, c.nextSibling.firstChild);return
			}
			f = c.parentNode.cloneNode(false);c.parentNode.insertBefore(f, c);f.appendChild(c);return
		}
	...
	_outdentMargin : function(d) {
		...
		if (d.tagName === "LI") {
			...
			if (!d.parentNode.tagName.match(/OL|UL/)) {
				...
				f = a.createElement("P");
				h = null;d.parentNode.insertBefore(f, d);
				while (d.firstChild) {
	...
	_insertBlankLine : function() {
		...
		while (c) {
			if (c.tagName == "BLOCKQUOTE") {
				this.elP1 = jindo.$("<p>&nbsp;</p>", this.oApp.getWYSIWYGDocument());c.parentNode.insertBefore(this.elP1, c);
				this.elP2 = jindo.$("<p>&nbsp;</p>", this.oApp.getWYSIWYGDocument());c.parentNode.insertBefore(this.elP2, c.nextSibling);break
			}
			c = c.parentNode
		}
		if (!this.elP1 && !this.elP2) {
			...
			if (b && b.tagName === "BLOCKQUOTE") {
				this.elP1 = jindo.$("<p>&nbsp;</p>", this.oApp.getWYSIWYGDocument());b.parentNode.insertBefore(this.elP1, b.nextSibling)
			}
			if (a && a.tagName === "BLOCKQUOTE") {
				this.elP1 = jindo.$("<p>&nbsp;</p>", this.oApp.getWYSIWYGDocument());a.parentNode.insertBefore(this.elP1, a)
			}
		}
	},
	_fixCorruptedBlockQuote : function(c) {
		...
		b.innerHTML = h.outerHTML.replace("<" + c, "<BLOCKQUOTE");h.parentNode.insertBefore(b.firstChild, h);h.parentNode.removeChild(h);
		l = e.childNodes[a];
		...
		d = this.oApp.getEmptySelection();d.selectNodeContents(l);d.collapseToEnd();d.select()
	},
	_removeBlockQuote : function() {
		...
		if (d && h) {
			a = h.parentNode;a.replaceChild(g, h);e.selectNodeContents(a);e.collapseToEnd();e.select()
		}
		while (a) {
	...
	_extendBlock : function() {
		...
		d.selectNode(f);d.select();return e
	}
});
nhn.husky.SE_WYSIWYGStyler = jindo.$Class({
	...
	$ON_SET_WYSIWYG_STYLE : function(c) {
		...
		if (b.collapsed && !g) {
			...
			if (q) {
				if (b.startContainer.tagName == "BODY" && b.startOffset === 0) {
					...
					if (h && p.nodeType == 1 && p.tagName == "BR") {
						b.selectNode(n);b.collapseToStart();b.insertNode(l)
					} else {
					...
					b.collapseToStart();b.insertNode(l)
				}
			} else {
			...
			b.selectNodeContents(q ? l : d);b.collapseToEnd();b._window.focus();b._window.document.body.focus();b.select();return
		}
		...
		if (g) {
			...
			for (var s = 0; s < r.length; s++) {
				b.selectNodeContents(r[s]);b.styleRange(c);b.select()
			}
		} else {
			var m = !!c.color;
...
nhn.husky.SE_WYSIWYGStyleGetter = jindo.$Class({
	...
	$ON_MSG_APP_READY : function() {
		this.oDocument = this.oApp.getWYSIWYGDocument();this.oApp.exec("ADD_APP_PROPERTY", [ "getCurrentStyle", jindo.$Fn(this.getCurrentStyle, this).bind() ]);
	...
	$ON_EVENT_EDITING_AREA_KEYPRESS : function(a) {
		...
		if (this.bAllSelected) {
			this.bAllSelected = false;return
		}
		this.oApp.exec("CHECK_STYLE_CHANGE")
	},
	$ON_EVENT_EDITING_AREA_KEYDOWN : function(a) {
...
nhn.husky.SE2M_FontSizeWithLayerUI = jindo.$Class({
	...
	$ON_MSG_STYLE_CHANGED : function(e, a) {
		if (e == "fontSize") {
			...
			if (d) {
				this.elFontSizeLabel.innerHTML = a;jindo.$Element(d).addClass("active")
			} else {
...
nhn.husky.SE2M_LineStyler = jindo.$Class({
	...
	$ON_SET_LINE_STYLE : function(a, c, d) {
		...
		this.setLineStyle(a, c, d, b);this.oApp.exec("CHECK_STYLE_CHANGE", [])
	},
	$ON_SET_LINE_BLOCK_STYLE : function(a, b, c) {
		this.oSelection = this.oApp.getSelection();this.setLineBlockStyle(a, b, c);this.oApp.exec("CHECK_STYLE_CHANGE", [])
	},
	_getSelectedTDs : function() {
		var a = {};
		this.oApp.exec("GET_SELECTED_TD_BLOCK", [ "aTdCells", a ]);return a.aTdCells || []
	},
	getLineStyle : function(e) {
		...
		setTimeout(jindo.$Fn(function(o, n) {
			...
			this.oSelection.setEndNodes(o, n);this.oSelection.select();this.oApp.exec("CHECK_STYLE_CHANGE", [])
		}, this).bind(a, c), 0);return h
	},
	...
	_getLineWrapper : function(d) {
		...
		g = l.oLineBreaker;
		h = f.oNode;
		e = f.oLineBreaker;this.oSelection.setEndNodes(j, h);
		if (g == e) {
...
nhn.husky.SE2M_ColorPalette = jindo.$Class({
	...
	$ON_EVENT_CLICK_COLOR_PALETTE : function(b) {
		...
		if (a.tagName && a.tagName == "BUTTON") {
			if (a == this.elMoreBtn) {
				this.oApp.exec("TOGGLE_COLOR_PICKER");return
			}
	...
	$ON_APPLY_COLOR : function(a) {
		...
		if (a == this.elOkBtn) {
			if (!this._verifyColorCode(c)) {
				this.elInputColorCode.value = "";alert(this.oApp.$MSG("SE_Color.invalidColorCode"));this.elInputColorCode.focus();return
			}
			this.oApp.exec("COLOR_PALETTE_APPLY_COLOR", [ c, true ]);return
		}
	...
	$ON_SHOW_COLOR_PICKER : function() {
		...
		this.$ON_SHOW_COLOR_PICKER = this._showColorPickerMain;this.$ON_SHOW_COLOR_PICKER()
	},
	...
	$ON_SHOW_COLOR_PALETTE : function(b, a) {
		this.sCallbackCmd = b;
		this.oLayerContainer = a;this.oLayerContainer.insertBefore(this.elColorPaletteLayer, null);
	...
	_initColor : function() {
		...
		this.elInputColorCode.value = "#000000";this.oApp.exec("HIDE_COLOR_PICKER")
	},
...
nhn.husky.SE2M_FontColor = jindo.$Class({
	...
	_assignHTMLElements : function(a) {
		...
		this.elPaletteHolder = jindo.$$.getSingle("DIV.husky_se2m_fontcolor_paletteHolder", this.elDropdownLayer);this._setLastUsedFontColor("#000000")
	},
	$BEFORE_MSG_APP_READY : function() {
...
nhn.husky.SE2M_BGColor = jindo.$Class({
	...
	_assignHTMLElements : function(a) {
		...
		this.elPaletteHolder = jindo.$$.getSingle("DIV.husky_se2m_BGColor_paletteHolder", this.elDropdownLayer);this._setLastUsedBGColor("#777777")
	},
	$BEFORE_MSG_APP_READY : function() {
...
nhn.husky.SE2M_Hyperlink = jindo.$Class({
	...
	$BEFORE_MSG_APP_READY : function() {
		...
		if (a && a.bAutolink === false) {
			...
			this.$ON_ENABLE_MESSAGE = null;try {
				this.oApp.getWYSIWYGDocument().execCommand("AutoUrlDetect", false, false)
			} catch (b) {}
		}
	},
	$ON_MSG_APP_READY : function() {
		this.bLayerShown = false;this.oApp.exec("REGISTER_UI_EVENT", [ "hyperlink", "click", "TOGGLE_HYPERLINK_LAYER" ]);this.oApp.exec("REGISTER_HOTKEY", [ "ctrl+k", "TOGGLE_HYPERLINK_LAYER", [] ]);this.oApp.registerLazyMessage([ "TOGGLE_HYPERLINK_LAYER", "APPLY_HYPERLINK" ], [ "hp_SE2M_Hyperlink$Lazy.js" ])
	},
	$ON_REGISTER_CONVERTERS : function() {
	...
	$LOCAL_BEFORE_FIRST : function(a) {
		if (!!a.match(/(REGISTER_CONVERTERS)/)) {
			this.oApp.acceptLocalBeforeFirstAgain(this, true);return true
		}
		this._assignHTMLElements(this.oApp.htOptions.elAppContainer);
		this.sRXATagMarker = this.sATagMarker.replace(/\//g, "\\/").replace(/\./g, "\\.");this.oApp.registerBrowserEvent(this.oBtnConfirm, "click", "APPLY_HYPERLINK");this.oApp.registerBrowserEvent(this.oBtnCancel, "click", "HIDE_ACTIVE_LAYER");this.oApp.registerBrowserEvent(this.oLinkInput, "keydown", "EVENT_HYPERLINK_KEYDOWN")
	},
	$ON_EVENT_HYPERLINK_KEYDOWN : function(a) {
		if (a.key().enter) {
			this.oApp.exec("APPLY_HYPERLINK");a.stop()
		}
	},
	...
	irToDb : function(k) {
		...
		for (var r = 0, m = d.length; r < m; r++) {
			...
			u.innerHTML = "";try {
				...
				u.innerHTML = (b + u.innerHTML).replace(/(&nbsp|\s)?(((?!http[s]?:\/\/)www\.(?:(?!\&nbsp;|\s|"|').)+)|(http[s]?:\/\/(?:(?!&nbsp;|\s|"|').)+))/ig, this._generateAutoLink);t[r].parentNode.insertBefore(u, t[r]);t[r].parentNode.removeChild(t[r])
			} catch (f) {}
			while (u.firstChild) {
...
nhn.husky.SE2M_FontNameWithLayerUI = jindo.$Class({
	...
	_rxComma : /\s*,\s*/g,
	$init : function(b, a) {
		this.elLastHover = null;this._assignHTMLElements(b);
		this.htBrowser = jindo.$Agent().navigator();
	...
	addAllFonts : function() {
		...
		if (!this.htOptions || !this.htOptions.aDefaultFontList || this.htOptions.aDefaultFontList.length === 0) {
			this.addFont("돋움,Dotum", "돋움", 0, "", "", 1, null, true);this.addFont("돋움체,DotumChe,AppleGothic", "돋움체", 0, "", "", 1, null, true);this.addFont("굴림,Gulim", "굴림", 0, "", "", 1, null, true);this.addFont("굴림체,GulimChe", "굴림체", 0, "", "", 1, null, true);this.addFont("바탕,Batang,AppleMyungjo", "바탕", 0, "", "", 1, null, true);this.addFont("바탕체,BatangChe", "바탕체", 0, "", "", 1, null, true);this.addFont("궁서,Gungsuh,GungSeo", "궁서", 0, "", "", 1, null, true);this.addFont("Arial", "Arial", 0, "", "", 1, "abcd", true);this.addFont("Tahoma", "Tahoma", 0, "", "", 1, "abcd", true);this.addFont("Times New Roman", "Times New Roman", 0, "", "", 1, "abcd", true);this.addFont("Verdana", "Verdana", 0, "", "", 1, "abcd", true);this.addFont("Courier New", "Courier New", 0, "", "", 1, "abcd", true)
		}
	...
	$ON_MSG_APP_READY : function() {
		this.bDoNotRecordUndo = false;this.oApp.exec("ADD_APP_PROPERTY", [ "addFont", jindo.$Fn(this.addFont, this).bind() ]);this.oApp.exec("ADD_APP_PROPERTY", [ "addFontInUse", jindo.$Fn(this.addFontInUse, this).bind() ]);this.oApp.exec("ADD_APP_PROPERTY", [ "setMainFont", jindo.$Fn(this.setMainFont, this).bind() ]);this.oApp.exec("ADD_APP_PROPERTY", [ "setDefaultFont", jindo.$Fn(this.setDefaultFont, this).bind() ]);this.oApp.exec("REGISTER_UI_EVENT", [ "fontName", "click", "SE2M_TOGGLE_FONTNAME_LAYER" ])
	},
	$AFTER_MSG_APP_READY : function() {
		this._initFontName();this._attachIEEvent()
	},
	_assignHTMLElements : function(a) {
	...
	_initFontName : function() {
		this._addFontInMarkup();this.addAllFonts();
		var a;
		if (this.oApp.getCurrentStyle && (a = this.oApp.getCurrentStyle())) {
			this.$ON_MSG_STYLE_CHANGED("fontFamily", a.fontFamily)
		}
		this.oApp.registerBrowserEvent(this.oDropdownLayer, "mouseover", "EVENT_FONTNAME_LAYER_MOUSEOVER", []);this.oApp.registerBrowserEvent(this.oDropdownLayer, "click", "EVENT_FONTNAME_LAYER_CLICKED", [])
	},
	_checkFontLI : function(c, b) {
		...
		c.style.display = a ? "block" : "none";return a
	},
	...
	_attachIEEvent : function() {
		...
		if (this.htBrowser.nativeVersion < 9) {
			this._wfOnPasteWYSIWYGBody = jindo.$Fn(this._onPasteWYSIWYGBody, this);this._wfOnPasteWYSIWYGBody.attach(this.oApp.getWYSIWYGDocument().body, "paste");return
		}
		if (document.documentMode < 9) {
			this._wfOnFocusWYSIWYGBody = jindo.$Fn(this._onFocusWYSIWYGBody, this);this._wfOnFocusWYSIWYGBody.attach(this.oApp.getWYSIWYGDocument().body, "focus");return
		}
	...
	_onMouseupCover : function(f) {
		...
		var g = f.mouse(),
			...
			a = jindo.$Element(c),
			b = this.oApp.getEmptySelection();
		b.selectNode(c);b.collapseToStart();b.select();a.fireEvent("mousedown", {
			left : g.left,
			middle : g.middle,
			right : g.right
		});a.fireEvent("mouseup", {
		...
		if (this.oApp.oNavigator.ie && document.documentMode < 11 && this.oApp.getEditingMode() === "WYSIWYG") {
			if (this.oApp.getWYSIWYGDocument().body.innerHTML == "<p></p>") {
				...
				var b = this.oApp.getSelection();
				b.moveToStringBookmark("INIT");b.select();b.removeStringBookmark("INIT")
			}
		} else {
			if (this.oApp.oNavigator.ie && this.oApp.oNavigator.nativeVersion == 11 && document.documentMode == 11 && this.oApp.getEditingMode() === "WYSIWYG") {
				if (this.oApp.getWYSIWYGDocument().body.innerHTML == "<p><br></p>") {
					...
					b.setStartBefore(d);b.setEndBefore(d);b.select()
				}
			}
		}
	},
	...
	_addFontToMenu : function(c, b, d) {
		...
		a.innerHTML = this.elFontItemTemplate.innerHTML.replace("@DisplayName@", c).replace("FontFamily", b).replace("@SampleText@", d);this.elFontNameList.insertBefore(a, this.elFontItemTemplate);
	...
	$ON_MSG_STYLE_CHANGED : function(d, a) {
		if (d == "fontFamily") {
			...
			if (b) {
				this.elFontNameLabel.innerHTML = this._getFontNameLabelFromLI(b);jindo.$Element(b).addClass("active")
			} else {
	...
	$ON_SET_FONTFAMILY : function(c, b) {
		...
		this.bDoNotRecordUndo = false;this.oApp.exec("RECORD_UNDO_AFTER_ACTION", [ "SET FONTFAMILY", {
			bMustBlockElement : true
		} ]);this.oApp.exec("HIDE_ACTIVE_LAYER", []);this.oApp.exec("CHECK_STYLE_CHANGE", [])
	},
	...
	addFont : function(d, b, k, c, e, h, g, l) {
		...
		g = g || this.oApp.$MSG("SE2M_FontNameWithLayerUI.sSampleText");this._addFontToMenu(a, f, g);
		if (!h) {
	...
	addFontInUse : function(c, d, g, e, b, f) {
		...
		a.loadCSS(this.oApp.getWYSIWYGDocument());return a
	},
	setMainFont : function(c, d, g, e, b, f) {
		...
		this.setDefaultFont(a.fontFamily, g);return a
	},
		...
		this._importCSS = function(h) {
			...
			if (g === 0 || f.imports.length == 30) {
				if (h.createStyleSheet) {
					...
					f = h.createElement("style");h.documentElement.firstChild.appendChild(f);
					f = f.sheet
				}
			}
...
nhn.ColorPicker = jindo.$Class({
	...
	hsv : function(d) {
		...
		var j = 0,
			g = 0;
		if (this.huePanel) {
			f = this._hsv2rgb(d.h, 100, 100);this.elem.css("background", "#" + this._rgb2hex(f.r, f.g, f.b));
			j = d.s / 100 * k;
			g = (100 - d.v) / 100 * c
		} else {
			var e = k / 2;
		...
		g = Math.max(Math.min(g - 1, c - a), 1);this.cursor.css({
			left : j + "px",
			top : g + "px"
		});
		this._hsvColor = d;
		f = this._hsv2rgb(d.h, d.s, d.v);this.fireEvent("colorchange", {
			type : "colorchange",
			element : this,
			currentElement : this,
			rgbColor : f,
	...
	paint : function() {
		if (this.huePanel) {
			this["_paintColWith" + this.canvasType]();this["_paintHueWith" + this.canvasType]()
		} else {
			this["_paintOneWith" + this.canvasType]()
		}
		this.cursor.appendTo(this.elem);this.cursor.css({
			position : "absolute",
			top : "1px",
			left : "1px",
			background : "white",
	...
	_paintColWithCanvas : function() {
		...
		var a = e.width();
		var c = e.height();
		d = b.createLinearGradient(0, 0, a, 0);d.addColorStop(0, "rgba(255,255,255,1)");d.addColorStop(1, "rgba(255,255,255,0)");
		b.fillStyle = d;b.fillRect(0, 0, a, c);
		d = b.createLinearGradient(0, 0, 0, c);d.addColorStop(0, "rgba(0,0,0,0)");d.addColorStop(1, "rgba(0,0,0,1)");
		b.fillStyle = d;b.fillRect(0, 0, a, c)
	},
	_paintOneWithFilter : function() {
		...
		for (var c = 1; c < 7; c++) {
			...
			j = "#FF" + this._rgb2hex(f.r, f.g, f.b);jindo.$Element("<div>").css({
				position : "absolute",
				left : 0,
				width : "100%",
				top : g + "px",
	...
	_paintOneWithCanvas : function() {
		...
		for (var d = 0; d < 7; d++) {
			c = this._hsv2rgb(d / 6 * 360, 100, 100);f.addColorStop(d / 6, "rgb(" + c.join(",") + ")")
		}
		b.fillStyle = f;b.fillRect(0, 0, a, e);
		f = b.createLinearGradient(0, 0, a, 0);f.addColorStop(0, "rgba(255,255,255,1)");f.addColorStop(0.5, "rgba(255,255,255,0)");f.addColorStop(0.5, "rgba(0,0,0,0)");f.addColorStop(1, "rgba(0,0,0,1)");
		b.fillStyle = f;b.fillRect(0, 0, a, e)
	},
	_paintHueWithFilter : function() {
		...
		for (var e = 1; e < 7; e++) {
			...
			d.appendTo(this.huePanel);d.css(k ? "left" : "top", 0).css(k ? "width" : "height", "100%");d.css(k ? "top" : "left", b + "px").css(k ? "height" : "width", a + "px")
		}
	},
	_paintHueWithCanvas : function() {
		...
		a.fillStyle = f;a.fillRect(0, 0, g.width(), g.height())
	},
	_rgb2hsv : function(k, j, a) {
		...
		f = Math.floor(f);
		e = Math.floor(e * 100);
		c = Math.floor(c / 255 * 100);return this._hsv(f, e, c)
	},
	_hsv2rgb : function(j, u, n) {
		...
		a = Math.floor(a * 255);
		k = Math.floor(k * 255);
		m = Math.floor(m * 255);return this._rgb(a, k, m)
	},
	...
	_onDownColor : function(a) {
		...
		this._colLayerPos = [ b.layerX, b.layerY ];this._onUpColorFn.attach(document, "mouseup");this._onMoveColorFn.attach(document, "mousemove");this._onMoveColor(a)
	},
	...
	_onDownHue : function(a) {
		...
		this._hueLayerPos = [ b.layerX, b.layerY ];this._onUpHueFn.attach(document, "mouseup");this._onMoveHueFn.attach(document, "mousemove");this._onMoveHue(a)
	},
	_onUpHue : function(a) {
	...
	_onMoveHue : function(d) {
		...
		c.h = c[0] = (Math.min(Math.max(f, 0), b) / b * 360) % 360;this.hsv(c);d.stop()
	}
}).extend(jindo.Component);
nhn.husky.SE2M_Accessibility = jindo.$Class({
	...
	$ON_MSG_APP_READY : function() {
		this.htAccessOption = nhn.husky.SE2M_Configuration.SE2M_Accessibility || {};this.oApp.exec("REGISTER_HOTKEY", [ "alt+F10", "FOCUS_TOOLBAR_AREA", [] ]);this.oApp.exec("REGISTER_HOTKEY", [ "alt+COMMA", "FOCUS_BEFORE_ELEMENT", [] ]);this.oApp.exec("REGISTER_HOTKEY", [ "alt+PERIOD", "FOCUS_NEXT_ELEMENT", [] ]);
		if ((this.sType == "basic" || this.sType == "light") && (this.sLang != "ko_KR")) {
			return
		} else {
			this.oApp.exec("REGISTER_HOTKEY", [ "alt+0", "OPEN_HELP_POPUP", [] ]);this.oApp.exec("REGISTER_HOTKEY", [ "esc", "CLOSE_HELP_POPUP", [], document ])
		}
	...
	$ON_FOCUS_NEXT_ELEMENT : function() {
		...
		if (this._currentNextFocusElement) {
			window.focus();this._currentNextFocusElement.focus()
		} else {
	...
	_findNextFocusElement : function(a) {
		...
		while (b) {
			...
			if (this._nextFocusElement) {
				...
				this._nextFocusElement = null;break
			} else {
				b = this._switchToSiblingOrNothing(b);
				if (!b) {
					break
	...
	$ON_FOCUS_BEFORE_ELEMENT : function() {
		...
		if (this._currentPreviousFocusElement) {
			window.focus();this._currentPreviousFocusElement.focus()
		} else {
			if (parent && parent.nhn && parent.nhn.husky && parent.nhn.husky.EZCreator && parent.nhn.husky.EZCreator.elIFrame) {
				parent.focus();
	...
	_findPreviousFocusElement : function(a) {
		...
		while (b) {
			...
			if (this._previousFocusElement) {
				...
				this._bStopFindingNextElement = false;
				this._previousFocusElement = null;break
			} else {
				b = this._switchToSiblingOrNothing(b, true);
				if (!b) {
					break
	...
	$ON_FOCUS_TOOLBAR_AREA : function() {
		...
		if (this.oButton && !this.oButton.disabled) {
			window.focus();this.oButton.focus()
		}
	},
	$ON_OPEN_HELP_POPUP : function() {
		this.oApp.exec("DISABLE_ALL_UI", [ {
			aExceptions : [ "se2_accessibility" ]
		} ]);this.oApp.exec("SHOW_EDITING_AREA_COVER");this.oApp.exec("SELECT_UI", [ "se2_accessibility" ]);
		this.nCalcX = this.htTopLeftCorner.x + this.oApp.getEditingAreaWidth() - this.nLayerWidth;
		this.nCalcY = this.htTopLeftCorner.y - 30;this.oApp.exec("SHOW_DIALOG_LAYER", [ this.elHelpPopupLayer, {
...
nhn.husky.SE2M_FindReplacePlugin = jindo.$Class({
	...
	$ON_MSG_APP_READY : function() {
		this.oEditingWindow = this.oApp.getWYSIWYGWindow();this.oApp.exec("REGISTER_HOTKEY", [ "ctrl+f", "SHOW_FIND_LAYER", [] ]);this.oApp.exec("REGISTER_HOTKEY", [ "ctrl+h", "SHOW_REPLACE_LAYER", [] ]);this.oApp.exec("REGISTER_UI_EVENT", [ "findAndReplace", "click", "TOGGLE_FIND_REPLACE_LAYER" ]);this.oApp.registerLazyMessage([ "TOGGLE_FIND_REPLACE_LAYER", "SHOW_FIND_LAYER", "SHOW_REPLACE_LAYER", "SHOW_FIND_REPLACE_LAYER" ], [ "hp_SE2M_FindReplacePlugin$Lazy.js", "N_FindReplace.js" ])
	},
...
nhn.husky.SE2M_Quote = jindo.$Class({
	...
	$ON_REGISTER_CONVERTERS : function() {
		this.oApp.exec("ADD_CONVERTER", [ "DB_TO_IR", jindo.$Fn(function(a) {
			a = a.replace(/<(blockquote)[^>]*class=['"]?(se2_quote[0-9]+)['"]?[^>]*>/gi, "<$1 class=$2>");return a
		}, this).bind() ]);this.oApp.exec("ADD_CONVERTER", [ "IR_TO_DB", jindo.$Fn(function(a) {
	...
	$ON_EVENT_EDITING_AREA_KEYDOWN : function(c) {
		...
		a = this.oApp.getSelection();a.fixCommonAncestorContainer();
		b = this._findParentQuote(a.commonAncestorContainer);
		if (!b) {
			return
		}
		if (this._isBlankQuote(b)) {
			c.stop(jindo.$Event.CANCEL_DEFAULT);a.selectNode(b);a.collapseToStart();jindo.$Element(b).leave();a.select()
		}
	},
	$ON_EVENT_EDITING_AREA_KEYUP : function(c) {
		...
		if (!b.nextSibling) {
			...
			d.innerHTML = "&nbsp;";jindo.$Element(b).after(d);setTimeout(jindo.$Fn(function(e) {
				var f = e.placeStringBookmark();
				e.select();e.removeStringBookmark(f)
			}, this).bind(a), 0)
		}
	},
...
nhn.husky.SE2M_TableCreator = jindo.$Class({
	...
	irToDbDOM : function(e) {
		...
		for (var d = 0, c = f.length; d < c; d++) {
			...
			g = jindo.$$("tbody>tr>td", a);jindo.$A(g).forEach(function(k, j, h) {
				jindo.$Element(k).css({
					border : "",
					borderTop : "",
					borderRight : ""
	...
	$ON_DELETE_BLOCK_CONTENTS : function() {
		...
		for (var c = 0; c < this.nSelectedTD; c++) {
			jindo.$Element(this.htSelectedTD.aTdCells[c]).child(function(f) {
				a = jindo.$Element(f._element.parentNode);a.remove(f);
...
nhn.husky.SE2M_StyleRemover = jindo.$Class({
	...
	$ON_CHOOSE_REMOVE_STYLE : function(a) {
		...
		c = b.bIsSelectedTd;this.oApp.exec("RECORD_UNDO_BEFORE_ACTION", [ "REMOVE STYLE", {
			bMustBlockElement : true
		} ]);
		if (c) {
	...
	$ON_REMOVE_STYLE_IN_BLOCK : function(c) {
		var d = {};
		this.oSelection = this.oApp.getSelection();this.oApp.exec("GET_SELECTED_TD_BLOCK", [ "aTdCells", d ]);
		var a = d.aTdCells;
		for (var b = 0; b < a.length; b++) {
			this.oSelection.selectNodeContents(a[b]);this.oSelection.select();this.oApp.exec("REMOVE_STYLE", [])
		}
	},
	$ON_REMOVE_STYLE : function(e) {
		...
		this._removeStyle(b);e.moveToBookmark(f);
		var b = e.getNodes(true);
		for (var d = 0; d < b.length; d++) {
			var g = b[d];
			if (g.style && g.tagName != "BR" && g.tagName != "TD" && g.tagName != "TR" && g.tagName != "TBODY" && g.tagName != "TABLE") {
				g.removeAttribute("align");g.removeAttribute("style");
		...
		while (c) {
			oParent = nhn.DOMFix.parentNode(c);oParent.removeChild(c);
	...
	$ON_REMOVE_STYLE_AND_PASTE_HTML : function(h, g) {
		...
		e = this.oApp.getSelection();e.deleteContents();
		b = this.oApp.getWYSIWYGDocument().createElement("DIV");e.insertNode(b);
		if (!!f.webkit) {
			b.innerHTML = "&nbsp;"
		}
		e.selectNode(b);this.oApp.exec("REMOVE_STYLE", [ e ]);
		if (f.ie) {
			h += "<p>&nbsp;</p>"
		} else {
			if (f.firefox) {
		...
		e.selectNode(b);e.pasteHTML(h);
		a = e.getNodes() || [];
		for (var c = 0; c < a.length; c++) {
			if (!!a[c].tagName && a[c].tagName.toLowerCase() == "td") {
				d = a[c];e.selectNodeContents(d.firstChild || d);e.collapseToStart();e.select();break
			}
		}
		e.collapseToEnd();e.select();this.oApp.exec("FOCUS");
		if (!b) {
			b.parentNode.removeChild(b)
		}
		if (!g) {
	...
	_splitAndAppendAtTop : function(b) {
		...
		while (jindo.$Element(c.parentNode).css("display") == "inline") {
			...
			c = c.parentNode;e.insertBefore(a, e.firstChild);
			a = e
		}
		oTop = c.parentNode;oTop.insertBefore(d, c.nextSibling);oTop.insertBefore(a, d.nextSibling)
	},
				...
				for (var d = 0, e = b.length; d < e; d++) {
					if (b[d].className) {
						...
						if (b[d].getAttribute(this.TMP_BGC_ATTR)) {
							b[d].style.backgroundColor = b[d].getAttribute(this.TMP_BGC_ATTR);b[d].removeAttribute(this.TMP_BGC_ATTR)
						} else {
							if (b[d].getAttribute(this.TMP_BGIMG_ATTR)) {
								jindo.$Element(this.aCells[d]).css("backgroundImage", b[d].getAttribute(this.TMP_BGIMG_ATTR));b[d].removeAttribute(this.TMP_BGIMG_ATTR)
							}
						}
					}
				}
...
nhn.husky.SE2M_QuickEditor_Common = jindo.$Class({
	...
	$ON_MSG_APP_READY : function() {
		...
		if (!this._bUseConfig) {
			...
			this._sAddTextAjaxUrl = a.common.sAddTextAjaxUrl;this.getData()
		}
...
function Shortcut(e, c) {
	...
	if (typeof c === "undefined" && e.constructor == String) {
		b.set("document", e, document);return d.init(b.get("document"), e)
	} else {
		if (c.constructor == String && e.constructor == String) {
			b.set(c, e, jindo.$(c));return d.init(b.get(c), e)
		} else {
			if (c.constructor != String && e.constructor == String) {
				var a = "nonID" + new Date().getTime();
				a = Shortcut.Store.searchId(a, c);b.set(a, e, c);return d.init(b.get(a), e)
			}
		}
	}
...
Shortcut.Data = jindo.$Class({
	$init : function(a, c, b) {
		this.id = a;
		this.element = b;
		this.func = jindo.$Fn(this.fire, this).bind();Shortcut.Helper.bind(this.func, b, "attach");
		this.keys = {};
		this.keyStemp = {};this.createKey(c)
	},
	createKey : function(b) {
		...
		a.events = [];
		a.commonExceptions = [];
		a.stopDefalutBehavior = true;return this
	},
...
Shortcut.Action = {
	init : function(b, a) {
		this.dataInstance = b;
		this.rawKey = a;return this
	},
	addEvent : function(a) {
		this.dataInstance.addEvent(a, this.rawKey);return this
	},
	removeEvent : function(a) {
		this.dataInstance.removeEvent(a, this.rawKey);return this
	},
	addException : function(a) {
		this.dataInstance.addException(a, this.rawKey);return this
	},
	removeException : function(a) {
		this.dataInstance.removeException(a, this.rawKey);return this
	},
	startDefalutBehavior : function() {
		this.dataInstance.startDefalutBehavior(this.rawKey);return this
	},
	stopDefalutBehavior : function() {
		this.dataInstance.stopDefalutBehavior(this.rawKey);return this
	},
	resetElement : function() {
		Shortcut.Store.reset(this.dataInstance.id);return this
	},
	resetAll : function() {
		Shortcut.Store.allReset();return this
	}
};(function() {
...
nhn.husky.SE_UndoRedo = jindo.$Class({
	...
	$ON_MSG_APP_READY : function() {
		var a = jindo.$Agent().navigator();
		this.bIE = a.ie;
		this.bFF = a.firefox;this.oApp.exec("REGISTER_UI_EVENT", [ "undo", "click", "UNDO" ]);this.oApp.exec("REGISTER_UI_EVENT", [ "redo", "click", "REDO" ]);this.oApp.exec("REGISTER_HOTKEY", [ "ctrl+z", "UNDO" ]);this.oApp.exec("REGISTER_HOTKEY", [ "ctrl+y", "REDO" ])
	},
	$ON_UNDO : function() {
		...
		this.oApp.exec("RESTORE_UNDO_HISTORY", [ this.oCurStateIdx.nIdx, this.oCurStateIdx.nStep, b ]);this.oApp.exec("CHECK_STYLE_CHANGE", []);
		this.sLastKey = null
	},
	$ON_REDO : function() {
		...
		this.oApp.exec("RESTORE_UNDO_HISTORY", [ this.oCurStateIdx.nIdx, this.oCurStateIdx.nStep ]);this.oApp.exec("CHECK_STYLE_CHANGE", []);
		this.sLastKey = null
	},
	$ON_RECORD_UNDO_ACTION : function(a, b) {
		...
		b.bTwoStepAction = false;this._doRecordUndoHistory(a, b)
	},
	$ON_RECORD_UNDO_BEFORE_ACTION : function(a, b) {
		...
		b.nStep = 0;
		b.bSkipIfEqual = false;
		b.bTwoStepAction = true;this._doRecordUndoHistory(a, b)
	},
	...
	$ON_RESTORE_UNDO_HISTORY : function(b, m, c) {
		...
		if (c) {
			...
			a = d._evaluateXPath(n, d._document);try {
				a.innerHTML = h;
				k = a.innerHTML
			} catch (j) {
				this.oApp.getWYSIWYGDocument().body.innerHTML = f;
	...
	_doRecordUndoHistory : function(r, j) {
		...
		if (!t && s) {
			...
			x.appendChild(g);x.appendChild(l);
			u = g.innerHTML;
			h = l.innerHTML;
			g = null;
			l = null;
	...
	_doRecordUndoHistoryAt : function(a, h, g, c, e, j, f, b) {
		if (a.nStep !== 0) {
			...
			d.oUndoCallback[0] = f;
			d.oRedoCallback = [];
			d.oRedoCallback[0] = b;this.aUndoHistory.splice(a.nIdx, this._historyLength - a.nIdx, d);
			this._historyLength = this.aUndoHistory.length
		}
...
nhn.husky.Utils = jindo.$Class({
	...
	$ON_ATTACH_HOVER_EVENTS : function(c, j) {
		...
		for (var f = 0, g = c.length; f < g; f++) {
			var h = a(c[f]);
			d.attach(h, "mouseover");e.attach(h, "mouseout");d.attach(h, "focus");e.attach(h, "blur")
		}
	}
});
...
nhn.husky.ActiveLayerManager = jindo.$Class({
	...
	$ON_SHOW_ACTIVE_LAYER : function(a, d, b) {
		...
		this.oCurrentLayer = a;this.oApp.exec("ADD_APP_PROPERTY", [ "oToolBarLayer", this.oCurrentLayer ])
	},
...
nhn.husky.StringConverterManager = jindo.$Class({
	...
	applyConverter : function(c, j, h) {
		...
		h = h || document;this.oApp.exec("MSG_STRING_CONVERTER_STARTED", [ c, k ]);
		var e;
		j = k.sContents;
		...
		k.sContents = k.sContents.replace(f, "");return k.sContents
	},
	$ON_ADD_CONVERTER : function(b, a) {
		var c = this.oApp.aCallerStack;
		a.sPluginName = c[c.length - 2].name;this.addConverter(b, a)
	},
	$ON_ADD_CONVERTER_DOM : function(b, a) {
		var c = this.oApp.aCallerStack;
		a.sPluginName = c[c.length - 2].name;this.addConverter_DOM(b, a)
	},
...
nhn.husky.LazyLoader = jindo.$Class({
	...
	$LOCAL_BEFORE_ALL : function(b, d) {
		...
		if (a.nLoadingStatus == 2) {
			this[b] = function() {
				this._removeHandler(b);this.oApp.delayedExec(c, d, 0);return false
			};return true
		}
		a.bLoadingStatus = 1;(new jindo.$Ajax(a.sURL, {
	...
	_onload : function(a, c, b) {
		if (b._response.readyState == 4) {
			...
			this.htMsgInfo[a].nLoadingStatus = 2;this._removeHandler("$BEFORE_" + a);this.oApp.exec("sMsg", c)
		} else {
	...
	if (this._instance == null) {
		this._instance = new (function() {
			...
			this.openWindow = function(b) {
				...
				this.removePluginWin(d);this._whtPluginWin.add(this.getCorrectKey(this._whtPlugin, h.oApp), d);return d
			};
			...
			this.removePluginWin = function(c) {
				...
				if (b) {
					this._whtPluginWin.remove(b);this.removePluginWin(c)
				}
			}
		})()
	}
	this._instance.addPlugin("plugin_" + (this._pluginKeyCnt++), a);return nhn.husky.PopUpManager._instance
};
...
nhn.husky.SE2M_Utils = {
	...
	ellipsis : function(k, c, d, j) {
		...
		f = h;
		while (f < h * (j + 0.5)) {
			a += Math.max(Math.ceil((l - a) / 2), 1);e.html(b.substring(0, a) + d);
			f = g.height()
		}
		while (f > h * (j + 0.5)) {
			a--;e.html(b.substring(0, a) + d);
			f = g.height()
		}
	},
	ellipsisByPixel : function(j, c, h, e) {
		...
		if (typeof e == "undefined") {
			...
			f = g;
			while (f < h) {
				a += Math.max(Math.ceil((k - a) / 2), 1);d.html(b.substring(0, a) + c);
				f = d.width()
			}
			e = function() {
				return true
	...
	loadCSS : function(c, e) {
		...
		d.setAttribute("type", "text/css");d.setAttribute("rel", "stylesheet");d.setAttribute("href", c);
		if (e) {
			if ("onload" in d) {
				d.onload = function() {
					e()
				...
				d.onreadystatechange = function() {
					...
					d.setAttribute("_complete", true);e()
				}
			}
		}
		a.appendChild(d)
	...
	iframeAlignConverter : function(f, b) {
		...
		if (e == "DIV" || e == "P") {
			...
			g.html(h).attr("align", c);d.replace(g)
		}
	},
	getJsonDatafromXML : function(a) {
		...
		a = a.replace(/<(\?|\!-)[^>]*>/g, "");a.replace(l, f);return jindo.$Json(d)
	},
...
nhn.husky.AutoResizer = jindo.$Class({
	...
	$init : function(c, d) {
		...
		this.sOverflow = this.welTextArea.css("overflow");this.welTextArea.css("overflow", "hidden");
		while (b--) {
	...
	bind : function() {
		this.welCloneDiv = jindo.$Element(this.welHiddenDiv.$value().cloneNode(false));this.wfnResize.attach(this.welTextArea, "keyup");this.welCloneDiv.appendTo(this.elContainer);this._resize()
	},
	unbind : function() {
		this.wfnResize.detach(this.welTextArea, "keyup");this.welTextArea.css("overflow", this.sOverflow);
		if (this.welCloneDiv) {
	...
	_resize : function() {
		...
		this.sContents += "<br>";this.welCloneDiv.html(this.sContents);
...
StringBuffer.prototype.append = function(a) {
	this._aString.push(a);return this
};
...
StringBuffer.prototype.setLength = function(a) {
	...
	IsInstalledFont = function(g) {
		...
		document.body.removeChild(a);return e
	}
})();
nhn.husky.SE2B_CSSLoader = jindo.$Class({
		...
		if (!jindo.$Agent().navigator().ie) {
			...
			for (var b = 0, c = this.aDelayedLoadTrigger.length; b < c; b++) {
				...
				this["$BEFORE_" + this.aDelayedLoadTrigger[b]] = jindo.$Fn(function(d) {
					...
					e = e.splice(1, e.length - 1);return this.loadSE2CSS(d, e)
				}, this).bind(a)
			}
		}
	},
	loadSE2CSS : function(a, b) {
		...
		nhn.husky.SE2M_Utils.loadCSS(this.htOptions.sCSSBaseURI + "/smart_editor2_items.css", c);return false
	}
});
if (typeof window.nhn == "undefined") {
	window.nhn = {}
...
nhn.husky.SE_OuterIFrameControl = $Class({
	...
	oResizeGrip : null,
	$init : function(a) {
		this.aHeightChangeKeyMap = [ -100, 100, 500, -500, -1, -10, 1, 10 ];this._assignHTMLObjects(a);
	...
	_keydown : function(a) {
		...
		if (b.keyCode >= 33 && b.keyCode <= 40) {
			this.oApp.exec("MSG_EDITING_AREA_RESIZE_STARTED", []);this.oApp.exec("RESIZE_EDITING_AREA_BY", [ 0, this.aHeightChangeKeyMap[b.keyCode - 33] ]);this.oApp.exec("MSG_EDITING_AREA_RESIZE_ENDED", []);a.stop()
		}
	},
	_mousedown : function(a) {
		this.iStartHeight = a.pos().clientY;
		this.iStartHeightOffset = a.pos().layerY;this.$FnMouseMove.attach(document, "mousemove");this.$FnMouseMove_Parent.attach(parent.document, "mousemove");this.$FnMouseUp.attach(document, "mouseup");this.$FnMouseUp.attach(parent.document, "mouseup");
		this.iStartHeight = a.pos().clientY;this.oApp.exec("MSG_EDITING_AREA_RESIZE_STARTED", [ this.$FnMouseDown, this.$FnMouseMove, this.$FnMouseUp ])
	},
	_mousemove : function(b) {
		var a = b.pos().clientY - this.iStartHeight;
		this.oApp.exec("RESIZE_EDITING_AREA_BY", [ 0, a ])
...
nhn.husky.SE_ToolbarToggler = $Class({
	...
	$ON_SE_TOGGLE_TOOLBAR : function() {
		this.toolbarArea.style.display = (this.toolbarArea.style.display == "none") ? "block" : "none";this.oApp.exec("MSG_EDITING_AREA_SIZE_CHANGED", [])
	}
	...
	$ON_SHOW_FIND_REPLACE_LAYER : function() {
		this.bLayerShown = true;this.oApp.exec("DISABLE_ALL_UI", [ {
			aExceptions : [ "findAndReplace" ]
		} ]);this.oApp.exec("SELECT_UI", [ "findAndReplace" ]);this.oApp.exec("HIDE_ALL_DIALOG_LAYER", []);
		this.elDropdownLayer.style.top = this.nDefaultTop + "px";this.oApp.exec("SHOW_DIALOG_LAYER", [ this.elDropdownLayer, {
			elHandle : this.elTitle,
	...
	$ON_HIDE_FIND_REPLACE_LAYER : function() {
		this.oApp.exec("ENABLE_ALL_UI");this.oApp.exec("DESELECT_UI", [ "findAndReplace" ]);this.oApp.exec("HIDE_ALL_DIALOG_LAYER", []);
	...
	$ON_FIND_REPLACE_LAYER_SHOWN : function() {
		...
		if (this.bFindMode) {
			...
			this.oFindInput_Keyword.value = "";this.oFindInput_Keyword.focus()
		} else {
			this.oReplaceInput_Original.value = "_clear_";
			this.oReplaceInput_Original.value = "";
			this.oReplaceInput_Replacement.value = "";this.oReplaceInput_Original.focus()
		}
	...
	$ON_SHOW_FIND : function() {
		this.bFindMode = true;
		this.oFindInput_Keyword.value = this.oReplaceInput_Original.value;jindo.$Element(this.oFindTab).addClass("active");jindo.$Element(this.oReplaceTab).removeClass("active");jindo.$Element(this.oFindNextButton).removeClass("normal");jindo.$Element(this.oFindNextButton).addClass("strong");
		this.oFindInputSet.style.display = "block";
		this.oReplaceInputSet.style.display = "none";
		this.oReplaceButton.style.display = "none";
		this.oReplaceAllButton.style.display = "none";jindo.$Element(this.elDropdownLayer).removeClass("replace");jindo.$Element(this.elDropdownLayer).addClass("find")
	},
	$ON_SHOW_REPLACE : function() {
		this.bFindMode = false;
		this.oReplaceInput_Original.value = this.oFindInput_Keyword.value;jindo.$Element(this.oFindTab).removeClass("active");jindo.$Element(this.oReplaceTab).addClass("active");jindo.$Element(this.oFindNextButton).removeClass("strong");jindo.$Element(this.oFindNextButton).addClass("normal");
		this.oFindInputSet.style.display = "none";
		this.oReplaceInputSet.style.display = "block";
		this.oReplaceButton.style.display = "inline";
		this.oReplaceAllButton.style.display = "inline";jindo.$Element(this.elDropdownLayer).removeClass("find");jindo.$Element(this.elDropdownLayer).addClass("replace")
	},
	$ON_FIND : function() {
		var a;
		if (this.bFindMode) {
		...
		var b = this.oApp.getSelection();
		b.select();switch (this.oFindReplace.find(a, false)) {
		case 1:
			alert(this.oApp.$MSG("SE_FindReplace.keywordNotFound"));b.select();
			break;case 2:
	...
	$ON_REPLACE : function() {
		...
		this.oApp.exec("RECORD_UNDO_AFTER_ACTION", [ "REPLACE" ]);switch (d) {
		case 1:
		case 3:
			alert(this.oApp.$MSG("SE_FindReplace.keywordNotFound"));b.select();
			break;case 4:
	...
	$ON_REPLACE_ALL : function() {
		...
		if (a === 0) {
			alert(this.oApp.$MSG("SE_FindReplace.replaceKeywordNotFound"));c.select();this.oApp.exec("FOCUS")
		} else {
			if (a < 0) {
				alert(this.oApp.$MSG("SE_FindReplace.keywordMissing"));c.select()
			} else {
				alert(this.oApp.$MSG("SE_FindReplace.replaceAllResultP1") + a + this.oApp.$MSG("SE_FindReplace.replaceAllResultP2"));
				c = this.oApp.getEmptySelection();c.select();this.oApp.exec("FOCUS")
			}
		}
	}
	...
	_unwrapBlock : function(a) {
		...
		if (e) {
			c.selectNodeContents(e);c.collapseToEnd();c.select()
		}
	...
	_wrapBlock : function(w, f) {
		...
		b.setStartBefore(x);b.setEndAfter(c);
		...
		if (n) {
			x = n;b.setStartBefore(n)
		}
		n = x;b.fixCommonAncestorContainer();
		...
		if (k) {
			k.className = f;this._setStyle(k, this.htQuoteStyles_view[f]);return
		}
		...
		if (f) {
			l.className = f;this._setStyle(l, this.htQuoteStyles_view[f])
		}
		t.insertBefore(l, j);b.setStartAfter(l);b.setEndAfter(c);b.surroundContents(l);
		if (this._isExceedMaxDepth(l)) {
			alert(this.oApp.$MSG("SE2M_Quote.exceedMaxCount").replace("#MaxCount#", (this.nMaxLevel + 1)));this.oApp.exec("HIDE_ACTIVE_LAYER", []);
			p = l.nextSibling;
			m = l.parentNode;
			v = l.childNodes;
			g = [];jindo.$Element(l).leave();
		...
		if (l && l.parentNode && l.parentNode.tagName == "BODY" && !l.nextSibling) {
			...
			d.innerHTML = "&nbsp;";l.parentNode.insertBefore(d, l.nextSibling)
		}
		if (nhn.husky.SE2M_Utils.isBlankNode(l)) {
			l.innerHTML = "&nbsp;";b.selectNodeContents(l.firstChild);b.collapseToStart();b.select()
		}
		this.oApp.exec("REFRESH_WYSIWYG");setTimeout(jindo.$Fn(function(y) {
			r = y.placeStringBookmark();y.select();y.removeStringBookmark(r);this.oApp.exec("FOCUS")
		}, this).bind(b), 0);this.oApp.exec("RECORD_UNDO_AFTER_ACTION", [ "BLOCK QUOTE", {
			sSaveTarget : "BODY"
		} ]);return l
	},
	...
	$LOCAL_BEFORE_FIRST : function(c) {
		...
		jindo.$Fn(b, this).attach(this.oInsertButton, "click");this.oApp.exec("SET_SCHARACTER_LIST", [ this.charSet ]);
	...
	$ON_EVENT_SCHARACTER_KEYDOWN : function(a) {
		if (a.key().enter) {
			this.oApp.exec("INSERT_SCHARACTERS");a.stop()
		}
	},
	$ON_TOGGLE_SCHARACTER_LAYER : function() {
		this.oTextField.value = "";
		this.oSelection = this.oApp.getSelection();this.oApp.exec("TOGGLE_TOOLBAR_ACTIVE_LAYER", [ this.elDropdownLayer, null, "MSG_SCHARACTER_LAYER_SHOWN", [], "MSG_SCHARACTER_LAYER_HIDDEN", [ "" ] ]);this.oApp.exec("MSG_NOTIFY_CLICKCR", [ "symbol" ])
	},
	...
	$ON_EVENT_SCHARACTER_CLICKED : function(b) {
		...
		this.oApp.exec("SELECT_SCHARACTER", [ c ]);b.stop()
	},
	$ON_SELECT_SCHARACTER : function(a) {
		...
		if (this.oTextField.createTextRange) {
			...
			if (this.oTextField.selectionEnd) {
				this.oTextField.selectionEnd = this.oTextField.value.length;this.oTextField.focus()
			}
		}
	},
	...
	$ON_CHANGE_SCHARACTER_SET : function(b) {
		...
		this._drawSCharList(b);jindo.$Element(this.aLabel[b]).addClass("active")
	},
	$ON_SET_SCHARACTER_LIST : function(a) {
		this.charSet = a;
		this.bSCharSetDrawn = new Array(this.charSet.length);this._drawSCharList(0)
	},
	_drawSCharList : function(c) {
		...
		for (var d = 0; d < a; d++) {
			f[d] = jindo.$("<LI>");
			if (this.bIE) {
				b = jindo.$("<BUTTON>");b.setAttribute("type", "button")
			} else {
				b = jindo.$("<BUTTON>");
				b.type = "button"
			}
			e = jindo.$("<SPAN>");
			e.innerHTML = unescape(this.charSet[c][d]);b.appendChild(e);f[d].appendChild(b);
	...
	$ON_TABLE_SET_ROW_NUM : function(a, b) {
		...
		this.elText_row.value = a;this._showNewTable()
	},
	$ON_TABLE_SET_COLUMN_NUM : function(b, a) {
		...
		this.elText_col.value = b;this._showNewTable()
	},
	...
	$ON_TABLE_INSERT : function() {
		...
		b = this.oApp.getSelection();
		b = this._divideParagraph(b);this.oApp.exec("RECORD_UNDO_BEFORE_ACTION", [ "INSERT TABLE", {
			sSaveTarget : "BODY"
		} ]);
		g = this.oApp.getWYSIWYGDocument().createElement("DIV");b.deleteContents();b.insertNode(g);b.selectNode(g);this.oApp.exec("REMOVE_STYLE", [ b ]);
		if (a.ie && this.oApp.getWYSIWYGDocument().body.childNodes.length === 1 && this.oApp.getWYSIWYGDocument().body.firstChild === g) {
			...
			g.parentNode.insertBefore(e, g);g.parentNode.removeChild(g)
		}
		if (a.firefox) {
			h = this.oApp.getWYSIWYGDocument().createElement("BR");e.parentNode.insertBefore(h, e.nextSibling)
		} else {
			if (a.ie) {
				h = this.oApp.getWYSIWYGDocument().createElement("p");e.parentNode.insertBefore(h, e.nextSibling)
			}
		}
	...
	_divideParagraph : function(f) {
		...
		if (!a.firstChild || nhn.husky.SE2M_Utils.isBlankNode(a)) {
			f.selectNode(a);f.select();return f
		}
		l = f.placeStringBookmark();f.moveToBookmark(l);
		n = this.oApp.getWYSIWYGDocument().createElement("P");f.setStartBefore(a.firstChild);f.surroundContents(n);f.collapseToEnd();
		h = this.oApp.getWYSIWYGDocument().createElement("P");f.setEndAfter(a.lastChild);f.surroundContents(h);f.collapseToStart();f.removeStringBookmark(l);
		d = jindo.$Element(a);d.after(h);d.after(n);d.leave();
		f = this.oApp.getEmptySelection();f.setEndAfter(n);f.setStartBefore(h);f.select();return f
	},
	...
	$ON_TABLE_SHOW_BORDER_STYLE_LAYER : function() {
		...
		this.elLayer_borderStyle.style.display = "block";this._refresh()
	},
	$ON_TABLE_HIDE_BORDER_STYLE_LAYER : function() {
		this.elBtn_borderStyle.className = "se2_view_more";
		this.elLayer_borderStyle.style.display = "none";this._refresh()
	},
	...
	$ON_TABLE_SHOW_STYLE_LAYER : function() {
		...
		this.elLayer_tableStyle.style.display = "block";this._refresh()
	},
	$ON_TABLE_HIDE_STYLE_LAYER : function() {
		this.elBtn_tableStyle.className = "se2_view_more";
		this.elLayer_tableStyle.style.display = "none";this._refresh()
	},
	...
	$ON_TABLE_SHOW_BORDER_COLOR_PALLET : function() {
		this.oApp.exec("TABLE_CLOSE_ALL", []);this.welDropdownLayer.addClass("p1");this.welDropdownLayer.removeClass("p2");this.oApp.exec("SHOW_COLOR_PALETTE", [ "TABLE_SET_BORDER_COLOR_FROM_PALETTE", this.elPanel_borderColorPallet ]);
		this.elPanel_borderColorPallet.parentNode.style.display = "block"
	},
	$ON_TABLE_HIDE_BORDER_COLOR_PALLET : function() {
		this.welDropdownLayer.removeClass("p1");this.oApp.exec("HIDE_COLOR_PALETTE", []);
	...
	$ON_TABLE_SHOW_BGCOLOR_PALLET : function() {
		this.oApp.exec("TABLE_CLOSE_ALL", []);this.welDropdownLayer.removeClass("p1");this.welDropdownLayer.addClass("p2");this.oApp.exec("SHOW_COLOR_PALETTE", [ "TABLE_SET_BGCOLOR_FROM_PALETTE", this.elPanel_bgColorPallet ]);
		this.elPanel_bgColorPallet.parentNode.style.display = "block"
	},
	$ON_TABLE_HIDE_BGCOLOR_PALLET : function() {
		this.welDropdownLayer.removeClass("p2");this.oApp.exec("HIDE_COLOR_PALETTE", []);
		this.elPanel_bgColorPallet.parentNode.style.display = "none"
	},
	$ON_TABLE_SET_BORDER_COLOR_FROM_PALETTE : function(a) {
		this.oApp.exec("TABLE_SET_BORDER_COLOR", [ a ]);this.oApp.exec("TABLE_HIDE_BORDER_COLOR_PALLET", [])
	},
	...
	$ON_TABLE_SET_BGCOLOR_FROM_PALETTE : function(a) {
		this.oApp.exec("TABLE_SET_BGCOLOR", [ a ]);this.oApp.exec("TABLE_HIDE_BGCOLOR_PALLET", [])
	},
	...
	$ON_EVENT_TABLE_CREATE_KEYDOWN : function(a) {
		if (a.key().enter) {
			this.elBtn_apply.focus();this.oApp.exec("TABLE_INSERT");a.stop()
		}
	},
	...
	_selectBorderStyle : function(a) {
		...
		this.nBorderStyleIdx = a;this._drawTableDropdownLayer()
	},
	_selectTableStyle : function(a) {
		this.elPanel_tableStylePreview.className = "se2_t_style" + a;
		this.nTableStyleIdx = a;this._drawTableDropdownLayer()
	},
	_showNewTable : function() {
		...
		var b = a.firstChild;
		this.elTable_preview.parentNode.insertBefore(b, this.elTable_preview);this.elTable_preview.parentNode.removeChild(this.elTable_preview);
		this.elTable_preview = b;this._refresh()
	},
	...
	$LOCAL_BEFORE_FIRST : function(a) {
		...
		this.rxLastDigits = RegExp("([0-9]+)$");this._assignHTMLObjects();this.oApp.exec("SE2_ATTACH_HOVER_EVENTS", [ this.aElBtn_tableStyle ]);this.addCSSClass(this.CELL_SELECTION_CLASS, "background-color:#B4C9E9;");this._createCellResizeGrip();
	...
	$ON_EVENT_EDITING_AREA_KEYUP : function(a) {
		...
		if (b.keyCode == 229 || b.alt || b.ctrl || b.keyCode == 16) {
			...
			if (b.keyCode == 8 || b.keyCode == 46) {
				this.oApp.exec("DELETE_BLOCK_CONTENTS");a.stop()
			}
		}
		switch (this.nStatus) {
	...
	$ON_TABLE_QE_SELECT_TEMPLATE : function(e) {
		...
		this._changeTableEditorStatus(this.STATUS.S_0);this.oApp.exec("STYLE_TABLE", [ b, c[1] ]);
	...
	$ON_TABLE_QE_SET_BGC : function(c) {
		...
		for (var a = 0, b = this.aSelectedCells.length; a < b; a++) {
			this.aSelectedCells[a].setAttribute(this.TMP_BGC_ATTR, c);this.aSelectedCells[a].removeAttribute(this.TMP_BGIMG_ATTR)
		}
	...
	$ON_TABLE_QE_SET_IMG : function(e) {
		...
		if (a === "0") {
			for (var f = 0, h = this.aSelectedCells.length; f < h; f++) {
				jindo.$Element(this.aSelectedCells[f]).css("backgroundImage", "");this.aSelectedCells[f].removeAttribute(this.TMP_BGC_ATTR);this.aSelectedCells[f].removeAttribute(this.TMP_BGIMG_ATTR)
			}
		} else {
			...
			for (var d = 0, h = this.aSelectedCells.length; d < h; d++) {
				jindo.$Element(this.aSelectedCells[d]).css("backgroundImage", "url(http://static.se2.naver.com/static/img/" + c + ")");this.aSelectedCells[d].removeAttribute(this.TMP_BGC_ATTR);this.aSelectedCells[d].setAttribute(this.TMP_BGIMG_ATTR, "url(http://static.se2.naver.com/static/img/" + c + ")")
			}
		}
	...
	$ON_SETUP_TABLE_QE_MODE : function(d) {
		...
		if (this.nQEMode === 0) {
			...
			this.elPanelReviewBGArea.style.display = "none";jindo.$Element(this.elPanelTableBGArea).className("se2_qe2");
	...
	$ON_TE_MERGE_CELLS : function() {
		...
		g.setAttribute("colSpan", this.htSelectionEPos.x - this.htSelectionSPos.x + 1);g.setAttribute("rowSpan", this.htSelectionEPos.y - this.htSelectionSPos.y + 1);
		...
		this.htMap = this._getCellMapping(this.elSelectionStartTable);this._selectCells(this.htSelectionSPos, this.htSelectionEPos);this._showTableTemplate(this.elSelectionStartTable);this._addClassToSelection();
		this.sQEAction = "TABLE_CELL_MERGE";this.oApp.exec("SHOW_COMMON_QE")
	},
	...
	$ON_TE_DELETE_COLUMN : function() {
		...
		this._selectAll_Column();this._deleteSelectedCells();
		this.sQEAction = "DELETE_TABLE_COLUMN";this._changeTableEditorStatus(this.STATUS.S_0)
	},
	$ON_TE_DELETE_ROW : function() {
		if (this.aSelectedCells.length === 0) {
			return
		}
		this._selectAll_Row();this._deleteSelectedCells();
		this.sQEAction = "DELETE_TABLE_ROW";this._changeTableEditorStatus(this.STATUS.S_0)
	},
	$ON_TE_INSERT_COLUMN_RIGHT : function() {
		if (this.aSelectedCells.length === 0) {
			return
		}
		this._selectAll_Column();this._insertColumnAfter(this.htSelectionEPos.x)
	},
	...
	$ON_TE_SPLIT_COLUMN : function() {
		...
		for (var c = 0, l = this.aSelectedCells.length; c < l; c++) {
			...
			for (var j = 0; j < this.htMap[0].length;) {
				k = this.htMap[d.x][j];
				f = parseInt(k.getAttribute("colSpan"), 10) || 1;k.setAttribute("colSpan", f + 1);
		...
		for (var c = 0, l = this.aSelectedCells.length; c < l; c++) {
			k = this.aSelectedCells[c];
			f = parseInt(k.getAttribute("colSpan"), 10) || 1;
			b = (f / 2).toFixed(0);k.setAttribute("colSpan", b);
			e = this._shallowCloneTD(k);e.setAttribute("colSpan", f - b);
			h = e;
			f = parseInt(k.getAttribute("rowSpan"), 10) || 1;e.setAttribute("rowSpan", f);
			e.innerHTML = "&nbsp;";
			g = k.width || k.style.width;
			if (g) {
				g = this.parseIntOr0(g);k.removeAttribute("width");
				m = (g / 2).toFixed();
				k.style.width = m + "px";
				e.style.width = (g - m) + "px"
			}
			k.parentNode.insertBefore(e, k.nextSibling);var a = jindo.$Agent().navigator();
		...
		this.htSelectionEPos.x = d.x;this._selectCells(this.htSelectionSPos, this.htSelectionEPos);
		this.sQEAction = "SPLIT_TABLE_COLUMN";this.oApp.exec("SHOW_COMMON_QE")
	},
	$ON_TE_SPLIT_ROW : function() {
		...
		for (var s = 0, l = this.aSelectedCells.length; s < l; s++) {
			...
			c = n[a.y];
			o = this.oApp.getWYSIWYGDocument().createElement("TR");c.parentNode.insertBefore(o, c.nextSibling);w++;
			for (var h = 0; h < this.htMap.length;) {
				r = this.htMap[h][a.y];
				v = parseInt(r.getAttribute("rowSpan"), 10) || 1;r.setAttribute("rowSpan", v + 1);
		...
		for (var s = 0, l = this.aSelectedCells.length; s < l; s++) {
			r = this.aSelectedCells[s];
			v = parseInt(r.getAttribute("rowSpan"), 10) || 1;
			d = (v / 2).toFixed(0);r.setAttribute("rowSpan", d);
			t = this._shallowCloneTD(r);t.setAttribute("rowSpan", v - d);
			v = parseInt(r.getAttribute("colSpan"), 10) || 1;t.setAttribute("colSpan", v);
			t.innerHTML = "&nbsp;";
			z = r.height || r.style.height;
			if (z) {
				z = this.parseIntOr0(z);r.removeAttribute("height");
			...
			b.insertBefore(t, e);var j = jindo.$Agent().navigator();
		...
		this._reassignCellSizes(this.elSelectionStartTable);
		this.htMap = this._getCellMapping(this.elSelectionStartTable);
		this.htSelectionEPos.y += w;this._selectCells(this.htSelectionSPos, this.htSelectionEPos);
		this.sQEAction = "SPLIT_TABLE_ROW";this.oApp.exec("SHOW_COMMON_QE")
	},
		...
		case this.STATUS.S_0:
			...
			if (b && this._rxCellNames.test(b.tagName)) {
				...
				if (!this._isValidTable(a)) {
					jindo.$Element(a).removeClass(this._sSETblClass);jindo.$Element(a).removeClass(this._sSEReviewTblClass);return
				}
				if (a) {
					this.elSelectionStartTD = b;
					this.elSelectionStartTable = a;this._changeTableEditorStatus(this.STATUS.MOUSEDOWN_CELL)
				}
			}
			break;case this.STATUS.MOUSEDOWN_CELL:
			break;case this.STATUS.CELL_SELECTING:
	...
	$ON_EVENT_EDITING_AREA_MOUSEMOVE : function(c) {
		...
		case this.STATUS.S_0:
			...
			if ((b && b !== this.elSelectionStartTD) || !b) {
				...
				this._reassignCellSizes(this.elSelectionStartTable);this._startCellSelection();this._selectBetweenCells(this.elSelectionStartTD, b)
			}
			break;case this.STATUS.CELL_SELECTING:
			var b = nhn.husky.SE2M_Utils.findClosestAncestorAmongTagNames(this._aCellName, c.element);
			...
			this.elLastSelectedTD = b;this._selectBetweenCells(this.elSelectionStartTD, b);
			break;case this.STATUS.CELL_SELECTED:
			break
		}
	},
		...
		case this.STATUS.CELL_SELECTING:
			...
			if (b < this.htEditingAreaPos.top) {
				...
				if (f > 0) {
					...
					var c = this.oApp.getSelection();
					c.selectNodeContents(this.aSelectedCells[0]);c.select();c.oBrowserSelection.selectNone()
				}
			} else {
				if (b > this.htEditingAreaPos.bottom) {
					var f = this.htSelectionEPos.y;
					if (f < this.htMap[0].length - 1) {
						this.htSelectionEPos.y++;this._selectCells(this.htSelectionSPos, this.htSelectionEPos);
						var c = this.oApp.getSelection();
						c.selectNodeContents(this.htMap[this.htSelectionEPos.x][this.htSelectionEPos.y]);c.select();c.oBrowserSelection.selectNone()
					}
				}
			}
			if (d < this.htEditingAreaPos.left) {
				...
				if (a > 0) {
					this.htSelectionSPos.x--;this._selectCells(this.htSelectionSPos, this.htSelectionEPos);
					var c = this.oApp.getSelection();
					c.selectNodeContents(this.aSelectedCells[0]);c.select();c.oBrowserSelection.selectNone()
				}
			} else {
				if (d > this.htEditingAreaPos.right) {
					...
					if (a < this.htMap.length - 1) {
						this.htSelectionEPos.x++;this._selectCells(this.htSelectionSPos, this.htSelectionEPos);
						var c = this.oApp.getSelection();
						c.selectNodeContents(this.htMap[this.htSelectionEPos.x][this.htSelectionEPos.y]);c.select();c.oBrowserSelection.selectNone()
					}
				}
	...
	_coverResizeLayer : function() {
		...
		this.elResizeCover.style.height = a.height - this.nPageTopBottomMargin + "px";document.body.appendChild(this.elResizeCover)
	},
	...
	_reassignCellSizes : function(t) {
		...
		for (var s = 0; s < c; s++) {
			var m = l[s];
			m[0].removeAttribute("width");m[0].removeAttribute("height");
			m[0].style.width = m[1];
	...
	_mousedown_ResizeCover : function(c) {
		...
		this.bResizingCover = true;this.wfnMousemove_ResizeCover.attach(this.elResizeCover, "mousemove");this.wfnMouseup_ResizeCover.attach(document, "mouseup");this._coverResizeLayer();
		...
		this.nClientYDiff = this.nStartHeight - this.htResizing.htEPos.clientY;this._reassignCellSizes(this.htResizing.elTable);
		...
		if (jindo.$Agent().navigator().chrome || jindo.$Agent().navigator().safari) {
			if (this.htResizing.nPreviousResizeMode != undefined && this.htResizing.nPreviousResizeMode != 0) {
				if (this.htResizing.nResizeMode != this.htResizing.nPreviousResizeMode) {
					this.htResizing.nResizeMode = this.htResizing.nPreviousResizeMode;this._showResizer()
				}
			}
		}
	...
	_mouseup_ResizeCover : function(h) {
		this.bResizing = false;this._hideResizer();this._uncoverResizeLayer();
		this.elResizeGrid.style.border = "";this.wfnMousemove_ResizeCover.detach(this.elResizeCover, "mousemove");this.wfnMouseup_ResizeCover.detach(document, "mouseup");
		var f = 0;
		var e = 0;
	...
	_changeTableEditorStatus : function(a) {
		...
		case this.STATUS.S_0:
			...
			this.oApp.exec("ENABLE_WYSIWYG", []);this.oApp.exec("CLOSE_QE_LAYER");this.elSelectionStartTable = null;
			break;case this.STATUS.CELL_SELECTING:
			if (this.oApp.oNavigator.ie) {
	...
	_getAllAffectedCells : function(d, a, o, m) {
		...
		if (o == 1) {
			for (var k = 0; k < n; k++) {
				...
				f[f.length] = h[d][k];var j = parseInt(h[d][k].style.width);
				if (b == -1 || b > j) {
					b = j
				}
			}
			if (h.length > d + 1) {
				for (var k = 0; k < n; k++) {
					...
					c[c.length] = h[d + 1][k];var j = parseInt(h[d + 1][k].style.width);
					if (e == -1 || e > j) {
						e = j
					}
				}
	...
	_createCellResizeGrip : function() {
		this.elTmp = document.createElement("DIV");try {
	...
	_selectAll_Row : function() {
		...
		this.htSelectionEPos.x = this.htMap.length - 1;this._selectCells(this.htSelectionSPos, this.htSelectionEPos)
	},
	_selectAll_Column : function() {
		this.htSelectionSPos.y = 0;
		this.htSelectionEPos.y = this.htMap[0].length - 1;this._selectCells(this.htSelectionSPos, this.htSelectionEPos)
	},
	_deleteSelectedCells : function() {
		var g;
		for (var b = 0, f = this.aSelectedCells.length; b < f; b++) {
			g = this.aSelectedCells[b];var e = jindo.$Agent().navigator();
	...
	_insertColumnAfter : function() {
		...
		for (var l = 0, o = this.htMap[0].length; l < o; l++) {
			...
			for (var m = this.htSelectionEPos.x; m >= this.htSelectionSPos.x; m--) {
				p = this.htMap[m][l];
				b = this._shallowCloneTD(p);var j = parseInt(p.getAttribute("rowSpan"));
				if (j > 1) {
				...
				for (var a = this.htSelectionEPos.x; a >= this.htSelectionSPos.x; a--) {
					if (this.htMap[a][l].parentNode == k) {
						g = this.htMap[a][l].nextSibling;break
					}
				}
				k.insertBefore(b, g);
		...
		this.htMap = this._getCellMapping(this.elSelectionStartTable);this._selectCells(this.htSelectionSPos, this.htSelectionEPos);this._showTableTemplate(this.elSelectionStartTable);this._addClassToSelection();
		this.sQEAction = "INSERT_TABLE_COLUMN";this.oApp.exec("SHOW_COMMON_QE")
	},
	_insertRowBelow : function() {
		...
		for (var o = this.htSelectionSPos.y; o <= this.htSelectionEPos.y; o++) {
			n = this._getTRCloneWithAllTD(o);l.insertBefore(n, g);
			if (c.ie && (c.nativeVersion >= 9 || c.nativeVersion <= 11) && (c.version >= 9 || c.version <= 11)) {
		...
		this.htMap = this._getCellMapping(this.elSelectionStartTable);this._selectCells(this.htSelectionSPos, this.htSelectionEPos);this._showTableTemplate(this.elSelectionStartTable);this._addClassToSelection();
		this.sQEAction = "INSERT_TABLE_ROW";this.oApp.exec("SHOW_COMMON_QE")
	},
	...
	_startCellSelection : function() {
		this.htMap = this._getCellMapping(this.elSelectionStartTable);this.oApp.getEmptySelection().oBrowserSelection.selectNone();
		if (this.oApp.oNavigator.safari || this.oApp.oNavigator.chrome) {
		...
		this.htEditingAreaPos.right = this.htEditingAreaPos.left + this.htEditingAreaPos.width;this._changeTableEditorStatus(this.STATUS.CELL_SELECTING)
	},
	_selectBetweenCells : function(b, a) {
		this._deselectCells();
		var f = this._getBasisCellPosition(b);
		var e = this._getBasisCellPosition(a);
		this._setEndPos(f);this._setEndPos(e);
		var d = {},
			c = {};
		d.x = Math.min(f.x, f.ex, e.x, e.ex);
		d.y = Math.min(f.y, f.ey, e.y, e.ey);
		c.x = Math.max(f.x, f.ex, e.x, e.ex);
		c.y = Math.max(f.y, f.ey, e.y, e.ey);this._selectCells(d, c)
	},
		...
		for (var h = 0, s = 0, d = r[0].length; h < d; h++, s++) {
			...
			if (!n[h].innerHTML.match(/TD|TH/i)) {
				for (var j = 0, a = r.length; j < a; j++) {
					...
					v = e;var c = parseInt(e.getAttribute("rowSpan"), 10) || 1;
					if (c > 1) {
	...
	_selectCells : function(b, a) {
		this.aSelectedCells = this._getSelectedCells(b, a);this._addClassToSelection()
	},
	...
	_addClassToSelection : function() {
		...
		for (var b = 0; b < this.aSelectedCells.length; b++) {
			...
			if (c) {
				...
				a = jindo.$Element(c);a.addClass(this.CELL_SELECTION_CLASS);a.addClass("undraggable");
				if (c.style.backgroundColor) {
					c.setAttribute(this.TMP_BGC_ATTR, c.style.backgroundColor);a.css("backgroundColor", "")
				}
				if (c.style.backgroundImage) {
					c.setAttribute(this.TMP_BGIMG_ATTR, c.style.backgroundImage);a.css("backgroundImage", "")
				}
			}
		}
	},
	_removeClassFromSelection : function() {
		...
		for (var b = 0; b < this.aSelectedCells.length; b++) {
			c = this.aSelectedCells[b];
			if (c) {
				a = jindo.$Element(c);a.removeClass(this.CELL_SELECTION_CLASS);a.removeClass("undraggable");
				if (c.getAttribute(this.TMP_BGC_ATTR)) {
					c.style.backgroundColor = c.getAttribute(this.TMP_BGC_ATTR);c.removeAttribute(this.TMP_BGC_ATTR)
				}
				if (c.getAttribute(this.TMP_BGIMG_ATTR)) {
					a.css("backgroundImage", c.getAttribute(this.TMP_BGIMG_ATTR));c.removeAttribute(this.TMP_BGIMG_ATTR)
				}
			}
		}
	},
		...
		if (e.y > 0) {
			for (b = e.x; b <= c.x; b++) {
				...
				if (this.htMap[b][e.y - 1] == d) {
					...
					e.y = a + 1;this._expandAndSelect(e, c);return
				}
			}
		}
		if (e.x > 0) {
			for (g = e.y; g <= c.y; g++) {
				...
				if (this.htMap[e.x - 1][g] == d) {
					...
					e.x = a + 1;this._expandAndSelect(e, c);return
				}
			}
		}
		if (c.y < this.htMap[0].length - 1) {
			for (b = e.x; b <= c.x; b++) {
				...
				if (this.htMap[b][c.y + 1] == d) {
					...
					c.y = a - 1;this._expandAndSelect(e, c);return
				}
			}
		}
		if (c.x < this.htMap.length - 1) {
			for (g = e.y; g <= c.y; g++) {
				...
				if (this.htMap[c.x + 1][g] == d) {
					...
					c.x = a - 1;this._expandAndSelect(e, c);return
				}
			}
		}
	},
	...
	_applyTableTemplate : function(b, a) {
		...
		this._clearAllTableStyles(b);this._doApplyTableTemplate(b, nhn.husky.SE2M_TableTemplate[a], false);b.setAttribute(this.ATTR_TBL_TEMPLATE, a)
	},
	_clearAllTableStyles : function(c) {
		c.removeAttribute("border");c.removeAttribute("cellPadding");c.removeAttribute("cellSpacing");
		c.style.padding = "";
		c.style.border = "";
		...
		if (k) {
			...
			for (var r = t, h = m.length; r < h;) {
				...
				if (c && c.tagName.match(/^TD|TH$/)) {
					d = parseInt(c.getAttribute("rowSpan"), 10) || 1;this._copyStylesTo(c, k, j)
				}
				r += d
			}
		}
	...
	_getTRCloneWithAllTD : function(d) {
		...
		for (var c = 0, f = this.htMap.length; c < f; c++) {
			...
			if (this._rxCellNames.test(e.tagName)) {
				b = this._shallowCloneTD(e);b.setAttribute("rowSpan", 1);b.setAttribute("colSpan", 1);
				b.style.width = "";
				b.style.height = "";a.insertBefore(b, null)
			}
		}
		return a
	},
	_shallowCloneTD : function(b) {
		...
		a.innerHTML = this.sEmptyTDSrc;return a
	},
	...
	addCSSClass : function(c, e) {
		...
		if (b.styleSheets[0] && b.styleSheets[0].addRule) {
			...
			a.appendChild(d);d.sheet.insertRule("." + c + " { " + e + " }", 0)
		}
	},
	...
	$ON_EVENT_APPLY_BGCOLOR : function(d) {
		...
		a = c.style.color;this.oApp.exec("APPLY_BGCOLOR", [ b, a ])
	},
	...
	$ON_APPLY_BGCOLOR : function(c, a) {
		if (!this.rxColorPattern.test(c)) {
			alert(this.oApp.$MSG("SE_Color.invalidColorCode"));return
		}
		...
		this.oApp.exec("SET_WYSIWYG_STYLE", [ b ]);this.oApp.exec("HIDE_ACTIVE_LAYER")
	}
	...
	$ON_APPLY_FONTCOLOR : function(a) {
		if (!this.rxColorPattern.test(a)) {
			alert(this.oApp.$MSG("SE_FontColor.invalidColorCode"));return
		}
		this._setLastUsedFontColor(a);this.oApp.exec("SET_WYSIWYG_STYLE", [ {
			color : a
		} ]);this.oApp.exec("HIDE_ACTIVE_LAYER")
	}
	...
	$ON_TOGGLE_HYPERLINK_LAYER : function() {
		...
		this.oApp.delayedExec("TOGGLE_TOOLBAR_ACTIVE_LAYER", [ this.oHyperlinkLayer, null, "MSG_HYPERLINK_LAYER_SHOWN", [], "MSG_HYPERLINK_LAYER_HIDDEN", [ "" ] ], 0);this.oApp.exec("MSG_NOTIFY_CLICKCR", [ "hyperlink" ])
	},
	$ON_MSG_HYPERLINK_LAYER_SHOWN : function() {
		...
		if (b && !this.oSelection.collapsed) {
			this.oSelection.selectNode(b);this.oSelection.select();
			var a = b.target;
			try {
		...
		this.oApp.delayedExec("SELECT_UI", [ "hyperlink" ], 0);this.oLinkInput.focus();
		this.oLinkInput.value = this.oLinkInput.value;this.oLinkInput.select()
	},
	$ON_MSG_HYPERLINK_LAYER_HIDDEN : function() {
		this.bLayerShown = false;this.oApp.exec("DESELECT_UI", [ "hyperlink" ])
	},
	...
	$ON_APPLY_HYPERLINK : function() {
		if (!this._validateTarget()) {
			alert(this.oApp.$MSG("SE_Hyperlink.invalidTarget"));return
		}
		...
		if (this._validateURL(h)) {
			sTarget = "_self";this.oApp.exec("RECORD_UNDO_BEFORE_ACTION", [ "HYPERLINK", {
			...
			if (this.oSelection.collapsed) {
				...
				g = this.oSelection.placeStringBookmark();this.oSelection.select();
				if (r.ie && (r.version === 8 || r.nativeVersion === 8)) {
					this.oApp.exec("IE_FOCUS", []);this.oSelection.moveToBookmark(g);this.oSelection.select()
				}
				...
				if (h == "") {
					...
					if (this._isExceptional()) {
						...
						jindo.$A(this.oSelection.getNodes(true)).forEach(function(u, e, w) {
							...
							if (u.nodeType === 3) {
								v.selectNode(u);v.pasteHTML(f + u.nodeValue + "</a>")
							} else {
								if (u.nodeType === 1 && u.tagName === "IMG") {
									v.selectNode(u);v.pasteHTML(f + jindo.$Element(u).outerHTML() + "</a>")
								}
							}
						}, this)
					} else {
			...
			this.oApp.exec("HIDE_ACTIVE_LAYER");setTimeout(jindo.$Fn(function() {
				...
				e.moveToBookmark(g);e.collapseToEnd();e.select();e.removeStringBookmark(g);this.oApp.exec("FOCUS");this.oApp.exec("RECORD_UNDO_AFTER_ACTION", [ "HYPERLINK", {
					sSaveTarget : (this.bModify ? "A" : null)
				} ])
			}, this).bind(), 17)
		} else {
			alert(this.oApp.$MSG("SE_Hyperlink.invalidURL"));this.oLinkInput.focus()
		}
	},
	...
	_validateURL : function(b) {
		...
		try {
			var a = b.split("?");
			a[0] = a[0].replace(/%[a-z0-9]{2}/gi, "U");decodeURIComponent(a[0])
		} catch (c) {
			return false
	...
	$ON_EVENT_SE2M_LINEHEIGHT_KEYDOWN : function(a) {
		if (a.key().enter) {
			this.oApp.exec("SE2M_SET_LINEHEIGHT_FROM_DIRECT_INPUT");a.stop()
		}
	},
	...
	$ON_OPEN_QE_LAYER : function(b, a, g) {
		if (this.waHotkeys.length() > 0 && !this.waHotkeyLayers.has(a)) {
			...
			for (var c = 0, e = this.waHotkeys.length(); c < e; c++) {
				f = this.waHotkeys.get(c);this.oApp.exec("ADD_HOTKEY", [ f[0], f[1], f[2], a ])
			}
		}
		var d = g;
		if (d) {
			this.targetEle = b;
			this.currentEle = a;this.layer_show(d, b)
		}
	},
	$ON_CLOSE_QE_LAYER : function(a) {
		...
		this.oApp.exec("CLOSE_SUB_LAYER_QE");this.layer_hide(a)
	},
	...
	toggle : function(b, a) {
		...
		this._in_event = false;a.stop(jindo.$Event.CANCEL_DEFAULT)
	},
	...
	layer_hide : function(a) {
		this.setOpenType(this._currentType, false);jindo.$Element(jindo.$$.getSingle("._" + this._environmentData[this._currentType].type, this.currentEle)).hide()
	},
	lazy_common : function() {
		this.oApp.registerBrowserEvent(jindo.$(this._qe_wrap), "keyup", "QE_IN_KEYUP");this.oApp.registerBrowserEvent(jindo.$(this._qe_wrap), "mousedown", "QE_IN_MOUSEDOWN");this.oApp.registerBrowserEvent(jindo.$(this._qe_wrap), "mousewheel", "QE_IN_MOUSEWHEEL");
		this.lazy_common = function() {}
	},
	layer_show : function(c, a) {
		this._currentType = c;this.setOpenType(this._currentType, true);
		var b = jindo.$$.getSingle("._" + this._environmentData[this._currentType].type, this.currentEle);
		jindo.$Element(b).show().css(this.get_position_layer(a, b));this.lazy_common()
	},
	...
	$ON_SHOW_DIALOG_LAYER : function(a, b) {
		...
		if (c == -1) {
			...
			if (b) {
				d = this.aDraggableLayer[c];d.setOptions(b)
			}
			a.style.display = "block"
		}
		if (b.sOnShowMsg) {
...
nhn.FindReplace = jindo.$Class({
		...
		if (this.document.body.createTextRange) {
			try {
				...
				if (this.document.selection) {
					this._range = this.document.selection.createRangeCollection().item(0);this._range.collapse(false)
				} else {
					if (!this._range) {
				...
				h = this._range.findText(a, 1, c);this._range.select();return h
			} catch (f) {
				return false
			}
		}
	...
	findReset : function() {
		if (this.window.find) {
			this.window.getSelection().removeAllRanges();return
		}
		if (this.document.body.createTextRange) {
			this._range = this.document.body.createTextRange();this._range.collapse(true);this._range.select()
		}
	},
	...
	_replace : function(c, f, b, d, g, e) {
		...
		e = e || new nhn.HuskyRange(this.window);e.setFromSelection();
		b = b || false;
		var a,
			h = e.toString();
		if (b) {
		...
		e.select();return this.find(c, b, d, g)
	},
	replaceAll : function(e, h, j, b) {
		...
		g.setFromSelection();g.collapseToStart();
		var m = this.window.document.createElement("SPAN");
		m.innerHTML = unescape("%uFEFF");g.insertNode(m);g.select();
		...
		var f = function() {
			...
			n.setFromSelection();g.moveToBookmark(d);
		...
		g.moveToBookmark(d);g.deleteContents();g.removeStringBookmark(d);setTimeout(function() {
			...
			for (var r = 0, l = c.length; r < l; r++) {
				...
				for (var q = o.length - a; q >= 0; q--) {
					...
					if (d == t) {
						...
						b.setEnd(p, m, true, true);b.setStart(k[r][q][0], k[r][q][1], true);
...
nhn.DraggableLayer = jindo.$Class({
	$init : function(c, b) {
		this.elLayer = c;this.setOptions(b);
		...
		this.$FnMouseUp = jindo.$Fn(jindo.$Fn(this._mouseup, this).bind(c), this);this.$FnMouseDown.attach(this.elHandle, "mousedown");
	...
	_mousedown : function(b, a) {
		...
		this.MouseOffsetX = (a.pos().clientX - this.toInt(b.style.left) - this.aBasePosition.left);this.$FnMouseMove.attach(b.ownerDocument, "mousemove");this.$FnMouseUp.attach(b.ownerDocument, "mouseup");
	...
	_mouseup : function(b, a) {
		this.oOptions.fnOnDragEnd();this.$FnMouseMove.detach(b.ownerDocument, "mousemove");this.$FnMouseUp.detach(b.ownerDocument, "mouseup");
